    ...
    <div class="inpour-container">
      ...
      <div class="inpour-wrap">
        ...
        <ul class="inpour-list">
          <li class="fast-click" v-for="(price,index) in inpourList" :key="index" :class="{ 'active': price === selectPrice }" @click="selectPrice = price">
            <p>{{price * 10}} 欢朋币</p>
            <p>￥{{price}}</p>
          </li>
        </ul>
      ...
      <div class="pay-mode">
        ...
        <ul class="pay-mode-list">
          <li class="list-item fast-click" @click="payMode=1" v-if="isInWechat">
            <div class="item-wrap">
          ...
          <li class="list-item fast-click" @click="payMode=2">
            <div class="item-wrap">
      ...
      <div class="live-pay-wrap">
        <button class="live-pay-btn fast-click" @click="payToHP(payMode, selectPrice)">立即充值</button>
        <div class="pay-mode-btn fast-click" :class="{'check':agreement === true,'uncheck':agreement===false}" @click="agreement=!agreement"></div>
        <p>我已阅读并同意
              ...
              self.$ajax.post('/api/wxpay/status.php', qs.stringify({
                ...
                if (res.data.content) {
                  ...
                  if (res.data.content.step == 'wait') {
                    ...
                    //判断是否为首充
                    if(res.data.content.isFirstPay && res.data.content.isFirstPay == 1){
                      alert('充值成功');
                      sessionStorage.setItem('_isFirstPay', 1);
                      clearInterval(timer);
                      if(sessionStorage.getItem('ref_url')){
                        location.href = decodeURIComponent(sessionStorage.getItem('ref_url'));
                      }
                      return;
                    }
                    alert('充值成功');

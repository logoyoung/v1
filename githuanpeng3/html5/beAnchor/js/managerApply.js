...
$(function() {
 ...
 var pageSizeNum = 10;
  ...
  var managerApply = {
    ...
    applying: function(type) {
      ...
      $('#canselSubmit').off().on('click',function(e) {
      	...
      	ajaxRequest({url: requestUrl,data: requestData},function(data) {
      		$('.layer').hide();
      	    window.location.href = $conf.domain + 'mobile/beAnchor';
    ...
    applyTo: function() {
      ...
      $('#applyToBtn').off().on('click',function(e) {
        ...
        // 新增选项
        var gameId = $('#managerGame').val();
        var gameGrade = $('#managerGameGrade').val();
        var gameQQ = $('#managerQQ').val();
        var gameFace = $('#managerFace').find('input:checked').val();

      	if(!gameId) {
          layer.msg('请填写直播游戏');
          return null;
        }else if(!gameGrade) {
          layer.msg('请填写游戏段位或等级');
          return null;
        }else if(!gameQQ) {
          layer.msg('请填写QQ号');
          return null;
  ...
  (function tap() {
    var testTap = document.querySelectorAll('.tap');
    if(testTap && testTap.length>0) {
      for(var i=0;i<testTap.length;i++) {
        testTap[i].addEventListener('touchstart',function(){
          this.classList.add('black');
        });

        testTap[i].addEventListener('touchend',function(i) {
...
/*
 * @method 请求游戏可选择列表
 * @by  fanguang  2017-09-12
 */
 function getGameList() {
   var requestUrl = $conf.api + 'anchor/gameList.php';
   ajaxRequest({url: requestUrl},function(data) {
      var list = data.list;
      var tpl = '<option value="" selected style="display:none;">去选择</option>';
      $.each(list,function(i,v){
        tpl += '<option value="' + v.gameId + '">'
              + v.gameName + '</option>';
      });
      $('#managerGame').html(tpl);
      getGameList.flag = true;
   },function(failData){
    layer.msg(failData.desc);
   })
 }

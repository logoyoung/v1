...
function getMyVideoLists($uid, $gameid, $gametid, $order, $type, $page, $size, $db)
{
    ...
    if ($type == 0) {//待发布
        ...
        $vctime = date("Y-m-d H:i:s", strtotime("-15 day"));
        $total = $db->field('count(*) as total')->where("ctime >= '$vctime' and  status in(" . VIDEO_WAIT . ',' . VIDEO_UNPUBLISH . ") and " . $where . "")->select('video');
        $tcount = ($total[0]['total'] == 0) ? 1 : $total[0]['total'];
        $page = returnPage($tcount, $size, $page);
        $lives = $db->field('videoid,gameid,gametid,gamename,vfile,length,poster,title,ctime,status,upcount,orientation')->where("ctime >= '$vctime' and  status in(" . VIDEO_WAIT . ',' . VIDEO_UNPUBLISH . ")  and " . $where . "")->order("$order desc")->limit($page, $size)->select('video');
    }
//    if ($type == 1) {//审核中
...
//            $redisobj->set($cacheKey, jsone($res), 60); //加入缓存,第三个参数为缓存时间60s  
    ...
    if ($afterSortLength) {
        foreach ($afterSort as $v) {
            ...
            if ($v['poster']) {
                $iparam = sposter($v['poster']);
                $finallyLive['poster'] = $v['poster'] ? $iparam: '';
                $finallyLive['ispic'] = '1';
            } else {
            ...
            if ($v['vfile']) {
                $vparam = sfile($v['vfile']);
            }
            $finallyLive['videoUrl'] = ($v['vfile']) ? $vparam : '';
...
if ($result) {
    succ(array('list' => $result['list'], 'total' => $result['count']));
} else {
    succ(array('list' => array(), 'total' => ''));
}

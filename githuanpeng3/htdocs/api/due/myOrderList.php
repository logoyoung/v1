 ...
 * @version 1.0.0
 */
class myOrderList extends ApiCommon {

    //用户登录状态有误
    const ERROR_USER_LOGIN = 710001;
    //获取用户约玩订单列表失败
    const ERROR_MY_ORDER_LIST = 710002;

    public static $errorMsg = [
        self::ERROR_USER_LOGIN => '用户登录状态有误',
        self::ERROR_MY_ORDER_LIST => '获取用户约玩订单列表失败',
    ];
    private $_enc;
    private $_uid;
    private $_page;
    ...
    public function getMyOrderList() {
        ...
        $total = $dueOrderService->getTotalNumberByUid($this->_uid);
        if (empty($total)) {
        ...
        if ($orderData) {
            ...
            foreach ($orderData as $k => $v) {
                ...
                $gameName = $certObj->getGameNameByGameId($certData[$v['cert_id']]['game_id']);
                $pic = explode(",", $certData[$v['cert_id']]['pic_urls']);
                ...
                $orderData[$k]['userAction'] = $dueOrderService->getUserOrderAction(\lib\due\DueOrder::ORDER_USER_ROLE_TYPE_01_USER, $orderData[$k]['status']);
                $tmpMessage = $dueOrderService->getOrderStatusMessage($v['status'], \lib\due\DueOrder::ORDER_USER_ROLE_TYPE_01_USER, $orderData[$k]['reason']);
                $orderData[$k]['statusDes'] = $tmpMessage['order'];
                $orderData[$k]['reason'] = $tmpMessage['reason'];
                $orderData[$k]['nick'] = $userData[$orderData[$k]['cert_uid']]['nick'];
                $orderData[$k]['pic'] = $userData[$orderData[$k]['cert_uid']]['pic'];
    ...
    public function display() {
        ...
        $list = $this->getMyOrderList();
        if (!$list) {
            $code = self::ERROR_MY_ORDER_LIST;

...
function sendHpBeanGiftHistory($uid, $date, $size, $page, $db){
	...
	$sql = "select * from giftrecord where giftid = 31 and uid=$uid and ctime between '$start' and '$end' order by id desc limit $from, $size";
	$res = $db->query($sql);
	...
	$sql = "select count(*) as count from giftrecord where giftid = 31 and uid=$uid and ctime between '$start' and '$end' order by id desc";
	$res = $db->query($sql);
	$row = $res->fetch_assoc();
...
$date = $year . "-" . $month;
$record = sendHpBeanGiftHistory($uid, $date, $size, $page ,$db);

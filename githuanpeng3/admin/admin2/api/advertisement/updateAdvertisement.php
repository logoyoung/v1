 ...
 * @return bool
 */
function updateAdvertisement($id, $uid, $adtype, $location, $url, $poster, $luid, $db)
{
    $data = array(
        'type' => $adtype,
        'location' => $location,
    ...
    $res = $db->where("id=$id")->update('admin_advertisement', $data);
    if ($res !== false) {
        return true;
    } else {
        return false;
...
$res = updateAdvertisement($id, $uid, $adtype, $location, $url, $poster, $luid, $db);
if ($res) {
    succ();
} else {
    error(-1014);

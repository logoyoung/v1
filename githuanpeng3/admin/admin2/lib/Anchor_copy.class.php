...
class Anchor
{
	...
	public function anchorValidDay($uids, $month)
	{
		...
		if(is_array($uids)){
			...
			$sql = "select uid, count(length) as total from live_length_copy
	...
	public function anchorLiveLength($uids, $month)
	{
		...
		if(is_array($uids)){
			...
			$sql = "select uid, length, DATE_FORMAT(`date`, '%d') as day from live_length_copy

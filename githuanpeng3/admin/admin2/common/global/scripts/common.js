...
!function(){
    ...
    $conf.domain = 'http://' + document.domain + '/' + 'admin2/';
    $conf.api = $conf.domain + 'api/';
    
    if(document.domain == 'admin-pre.huanpeng.com'){
        $conf.api_upload = $conf.api;
    }else{
        $conf.api_upload = $conf.api;
    }
    ...
    $conf.message = {
        api:$conf.api + 'message/'
    }
    $conf.recommend = {
        api:$conf.api + 'recommend/'
    }
    $conf.statistic = {
        api: $conf.api + 'statistic/'
    }
    $conf.tool = {
        api: $conf.api + 'tool/'
    }
...
function rebuildVueData(conf, data){
    var obj = {};
	for(var i in conf){
        var index = conf[i];
		obj[index] = data[index] ? data[index]: null;
    }
    return obj;
}
...
function runSwfFunction(objName, callfunc){
    ...
    function doCallBack(fn, obj, args){
        if(objName=='imProxy' && fn =='login')
            console.log('call improxy login function in time ' + new Date().getTime());
        var func;
        try{
...
function ajaxRequest(conf, successFn, failedFn, errorFn){
    var defaultConf = {
        ...
        success:function(d){
            if(d.stat == 1){
                successFn && typeof successFn=="function" && successFn(d.resuData);
            }else{
                failedFn && typeof failedFn=='function' && failedFn(d.err);
            }
        }
    };

...
/***/ function(module, exports, __webpack_require__) {
	...
	var _nickCheckTable = __webpack_require__(45);

	var _nickCheckTable2 = _interopRequireDefault(_nickCheckTable);
		...
		new _Vue2.default({
			...
			components: {
				check: {
					...
					components: {
						nickCheckTable: _nickCheckTable2.default
					}
				},
				pass: {
...
/***/ function(module, exports, __webpack_require__) {
	...
	if (false) {(function () {  module.hot.accept()
	  ...
	  hotAPI.install(require("vue"), true)
	  if (!hotAPI.compatible) return
	  var id = "/Users/hantong/Sites/website/mobileGame/huanpeng/htdocs/admin2/vue/UserManage/userNick/nickCheckTable.vue"
	  if (!module.hot.data) {
	    hotAPI.createRecord(id, module.exports)
	  } else {
	    hotAPI.update(id, module.exports, __vue_template__)
	...
	exports.default = {
	    ...
	    data: function data() {
	        return {
	            ...
	            totalCounts: 1,
	            currentPage: 1,
	            params: ['checked', 'nick', 'uid', 'ctime', 'status'],
	            url: $conf.nick.api + 'getNickList.php',
	            passUrl: $conf.nick.api + 'setNick.php',
	            unpassUrl: $conf.nick.api + 'setNick.php',
	            reCheckUrl: $conf.nick.api + ''
	        };
	    },
	    ...
	    methods: {
	        ...
	        passed: function passed() {
	            var self = this;
	            var uids = this.getCheckedUids();
	            var data = {
	                uid: getCookie('admin_uid'),
	                encpass: getCookie('admin_enc'),
	                type: getCookie('admin_type'),
	                succuid: uids['select'].join(),
	                failuid: uids['unselect'].join()
	            };
	        ...
	        unpassed: function unpassed() {
	            ...
	            var data = {
	                ...
	                encpass: getCookie('admin_enc'),
	                type: getCookie('admin_type'),
	                succuid: uids['unselect'].join(),
	                faileduid: uids['select'].join()
	            };
	        ...
	        getCheckedUids: function getCheckedUids() {
	            var select = [];
	            var unselect = [];
	            for (var i in this.dataList) {
	                if (this.dataList[i].checked) {
	                    if (this.dataList[i].uid) select.push(this.dataList[i].uid);
	                } else {
	                    if (this.dataList[i].uid) unselect.push(this.dataList[i].uid);
	                }
	            }

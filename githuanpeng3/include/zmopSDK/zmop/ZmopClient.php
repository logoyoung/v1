...
class ZmopClient {
	...
	public function getExecuteParams($request){
		$apiParamsQuery = $this->getBizParamStr($request);
		$sysParams = $this->getSystemParams($request);

		$result[$this->signParam] = urlencode(RSAUtil::sign($apiParamsQuery, $this->privateKeyFilePath));
		$result[$this->bizParam] = urlencode(RSAUtil::rsaEncrypt($apiParamsQuery,$this->zhiMaPublicKeyFilePath));
	...
	public function execute($request) {
		...
		mylog('encrypetd api parrams is '.json_encode($encryptedApiParams),LOGFN_SEND_MSG_ERR);
		mylog('http url is '.$requestUrl, LOGFN_SEND_MSG_ERR);
		...
		if ("json" == $this->format) {
			...
			$this->logCommunicationError($sysParams['method'], $requestUrl, 'HTTP_RESPONSE_NOT_WELL_FORMED',json_encode($biz_response));
		...
		mylog('respObject values is '.json_encode($respObject),LOGFN_SEND_MSG_ERR);
		...
		mylog('vertify result is '.json_encode($verifyResult) ,LOGFN_SEND_MSG_ERR);

		//如果ZMOP返回了错误码，记录到业务错误日志中
		if (isset ($bizRespObject->error_code)) {
			$logger = new LtLogger;
			$logger->conf["log_file"] = rtrim(ZMOP_SDK_WORK_DIR, '\\/') . '/' . "zmop_biz_err_" . $this->appId . "_" . date("Y-m-d") . ".log";
			$logger->log(array(
					date("Y-m-d H:i:s"),
					$resp
				));
	...
	private function logCommunicationError($apiName, $requestUrl, $errorCode, $responseTxt) {
		...
		write_log("error|apiName:{$apiName};requestUrl:{$requestUrl};errorCode:{$errorCode};responseTxt{$responseTxt}");
		return ;
	}
}
...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		$inputObj->SetAppid(WxPayConfig::getConstValue('APPID'));//公众账号ID
		$inputObj->SetMch_id(WxPayConfig::getConstValue('MCHID'));//商户号

		//终端IP获取 由于正式机有前台机，会导致h5支付失败，这里启用自己的IP获取方法
		//$inputObj->SetSpbill_create_ip($_SERVER['REMOTE_ADDR']);//终端ip
		//
		$port = 0;
		$ip = fetch_real_ip($port);
		$inputObj->SetSpbill_create_ip($ip);
...
//		curl_setopt($ch,CURLOPT_SSL_VERIFYHOST,2);//严格校验
		...
		if($useCert == true){
			...
			curl_setopt($ch,CURLOPT_SSLCERT, WxPayConfig::getConstValue('SSLCERT_PATH'));//WxPayConfig::SSLCERT_PATH;
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			curl_setopt($ch,CURLOPT_SSLKEY, WxPayConfig::getConstValue('SSLKEY_PATH'));//WxPayConfig::SSLKEY_PATH
		}
		//post提交方式

...
class sendMobileMsg
{
    ...
    const devGetBalanceURL= 'http://dev.liveuser.6.cn/api/getBalanceInfo.php?appid=102';
    ...
    public static function getDevMsgBalanceByUrl(){
        $env = strtoupper($GLOBALS['env']);
        if ($env != "PRO") {
            $get_url = self::devGetBalanceURL;
        } else {
            $get_url = self::getBalanceURL;
        }

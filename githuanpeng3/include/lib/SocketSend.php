...
class SocketSend
{
	...
	private static function _buildSendUrl( array $content, DBHelperi_huanpeng $db )
	{
		...
		elseif( is_array($luid) )
		{
			$urls = [];
			foreach ($luid as $tmpLuid)
			{
				$tmpContent = $content;
				$tmpContent['luid'] = $tmpLuid;
				$tmpUrls = self::_buildSendUrl($tmpContent, $db);
				$urls = array_merge($urls, $tmpUrls);
			}
			return $urls;
		}
		else
	 ...
	 * @return bool
	 */
	public static function sendMsg( array $content, DBHelperi_huanpeng $db = null )
	{
		if ( !$db )
		{
			$db = static::_getDb();
	...
	private function _getDb()
	{
		if ( !static::$_db )
		{
			static::$_db = new DBHelperi_huanpeng();
		}

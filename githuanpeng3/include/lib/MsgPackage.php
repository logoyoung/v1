...
class MsgPackage
{
	 ...
	 * @return mixed
	 */
	public static function getLiveStartMsgSocketPackage( int $luid, int $liveid, int $liveType = 0 )
	{
	 ...
	 * @return mixed
	 */
	public static function getLiveEndMsgSocketPackage( int $luid, int $liveid, int $reasonid = 0, string $reason = '', $liveType = 0 )
	{
		$content = array(
			't'      => 602,
			'lid'    => $liveid,
...
//			'constamount' => $amount
	 ...
	 * @return mixed
	 */
	public static function getSendGiftMsgCallBackSocketPackage( int $luid, int $uid, int $e, int $mid, int $coin, int $bean, int $cost, $sendType = 0, $packBack = [] )
	{
	...
	public static function getRoomRankUpMsgSocketPackage( $luid, $uid, $rankType, $upType, $nick, $level, $pre, $cur )
	{
		$chartsNameList = [
			1  => "贡献日榜",
			5  => "贡献周榜",
		...
		$chartsName = $chartsNameList[$rankType];
		$msgList    = [
			1  => "入住{$chartsName}",
			10 => "{$chartsName}排名提升",
			20 => "成为{$chartsName}榜首"
	 ...
	 * @param $luid        房间所有者ID
	 * @param $uid         接受消息者ID
	 * @param $rewordType  奖励类型 1:观看奖励，2：直播奖励
	 * @param $rewordLevel 奖励等级 1:初级，5:中级， 10：高级
	 * @param $exp         奖励经验值
	 *
	 * @return mixed
	 */
	public static function getLiveLengthExpRewardMsgSocketPackage( $luid, $uid, $rewordType, $rewordLevel, $exp )
	{
		$typeNotice = [
			1 => '观看',
			2 => '直播'
		...
		$msg = "你已完成今日{$levelNotice[$rewordLevel]}{$typeNotice[$rewordType]}任务，经验值";

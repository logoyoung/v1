...
class Roomid
{
    ...
    public function add($roomid,$uid,$utime)
    {
        if(!$roomid || !$uid)
        {
            return false;
        ...
        $db  = $this->getDb();
        $sql = "INSERT INTO `{$this->getTable()}` (`uid`,`roomid`,`utime`) VALUES(:uid,:roomid,:utime)";
    ...
    public function getMaxRoomid()
    {
        try {
            ...
            $sql    = "SELECT MAX(`roomid`) as rid FROM `{$this->getTable()}`";
            $db     = $this->getDb();
            $result = $db->query($sql,[],true);
            if(!isset($result[0]['rid']) || !$result[0]['rid'])
            {
                return 1001;
            }

            return (int) $result[0]['rid'];

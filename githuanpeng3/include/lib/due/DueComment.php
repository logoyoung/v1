...
class DueComment {
    ...
    public function getCommentsByUids(array $uids) {
        $uidRule = $this->_db->buildInPrepare($uids); 
        $sql = "select `id`,`order_id`,`uid`,`cert_uid`,`skill_id`,`star`,`tag_ids`,`comment`,`ctime`,`utime`,`status` from `due_comment` where cert_uid in ({$uidRule}) {$this->fiterStatus()}";
        $bdParam = $uids;
        $result = $this->_db->query($sql,$bdParam);
        return $result;
    }
    ...
    public function updateAvg($skill_id, $avg, $total_score, $num) {
        $sql = "update `due_skill` set `avg_score` =:avg_score , `total_score`=:total_score, `comment_num`=:num  where id=:skill_id";
        $bindParam = [
            'avg_score'=>$avg,
        ...
        $result = $this->_db->query($sql,$bindParam);
        return $result;
    }

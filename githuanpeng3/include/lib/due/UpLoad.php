...
class UpLoad
{
    ...
    private $dirwrite = true; // 默认允许创建新目录
    ...
    public function checkName($file)
    {
        if (!in_array(strtolower(pathinfo($file, PATHINFO_EXTENSION)), $this->allowtype)) {
            $err = "errorType@FileUpLoad::checkName(the type of file you upload was invalid)";
            mylog($err);
     ...
     * @return boolean
     */
    public function checkType($mime)
    {
        if (!in_array(explode('/', $mime)[1], $this->allowtype)) {
            $err = "errorType@FileUpLoad::checkType({$mime})";
            mylog($err);
            $this->errcode = -6017; // 上传文件类型错误
            return false;
        }
        return true;
    }
    ...
    public function upSingleFile($singlefileobj)
    {
        ...
        //检查错误信息
       if (!$this->checkError($singlefileobj['error']))
            return false;
        //检查文件格式
        if (!$this->checkName($singlefileobj['name']))
            return false;
         //检查文件格式
        if (!$this->checkType($singlefileobj['type']))
            return false;
        //检查文件大小
        ...
        if (!move_uploaded_file($singlefileobj['tmp_name'], $this->path . $indexDir . $fileName)) {
            ...
            mylog($err . json_encode(error_get_last()));

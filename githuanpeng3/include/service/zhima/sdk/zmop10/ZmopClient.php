<?php
namespace service\zhima\sdk\zmop10;
use service\zhima\sdk\zmop10\RSAUtil;
use service\zhima\sdk\zmop10\WebUtil;
use Exception;
...
class ZmopClient {
	...
	public function execute($request) {
		...
		if (isset ($bizRespObject->error_code)) {
			write_log("zmop_biz_err|{$this->appId};".hp_json_encode($resp),$this->_log);
		}
	...
	private function logCommunicationError($apiName, $requestUrl, $errorCode, $responseTxt) {
		write_log("error|apiName:{$apiName};requestUrl:{$requestUrl};errorCode:{$errorCode};responseTxt{$responseTxt}",$this->_log);
	}
}
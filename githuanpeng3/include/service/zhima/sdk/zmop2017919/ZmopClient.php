...
use service\zhima\sdk\zmop2017919\RSAUtil;
use service\zhima\sdk\zmop2017919\WebUtil;
use Exception;
...
class ZmopClient {
	...
	function __construct($gatewayUrl = '', $appId = '', $charset = 'UTF-8', $privateKeyFilePath, $zhiMaPublicKeyFilePath,$httpTimeout = 7){
	...
	public function execute($request) {
		...
		//发起HTTP请求
		try {
			$resp = WebUtil::curl($requestUrl, $encryptedApiParams , $this->_httpTimeout, $this->_httpTimeout);
		} catch (Exception $e) {
			write_log("error|http_error_code:$e->getCode(); http_error_msg:{$e->getMessage()}; {$sysParams['method']}.{$requestUrl};class:".__CLASS__.'line:'.__LINE__, $this->_log);
			return false;
		}
		...
		if (isset ($bizRespObject->error_code)) {
			write_log("error|zmop_biz_err;{$this->appId};".hp_json_encode($resp).';class:'.__CLASS__.'line:'.__LINE__,$this->_log);
		}
	...
	private function logCommunicationError($apiName, $requestUrl, $errorCode, $responseTxt) {
		write_log("error|apiName:{$apiName};requestUrl:{$requestUrl};errorCode:{$errorCode};responseTxt{$responseTxt};class:".__CLASS__,$this->_log);
	}
}
...
class UserRedis
{
    ...
    const USER_REAL_NAME    = 'userrealname';
    const USER_PASS_NAME    = 'password';
    const USER_ENC_NAME     = 'encpass';
    const USER_IS_EXSITS    = 'exist';
    const USER_CERT_STATUS  = 'cert_status';
    const USER_CERT_ZHIMA   = 'cert_zhima_status';
    ...
    public function isExist()
    {
        ...
        if(!$this->_uid)
        {
            $this->_uid = $this->getUidByPhone();
            $key = $this->getUserDataKey($this->_uid);
        }

        if($this->getRedis()->hExists($key,self::USER_IS_EXSITS) === false)
        {
            return -1;
        }
        $status = $this->getRedis()->hGet($key,self::USER_IS_EXSITS);
    ...
    public function setUserRealNameData(array $data)
    {
        if(!$this->_uid)
        {
            return false;
        ...
        $colunm[self::USER_REAL_NAME] = hp_json_encode(array_values_to_string($data));
    ...
    /**
     * 更新用户实名认证状态
     * @param  integer $status 1 认证通过，0没通过
     * @return [type]          [description]
     */
    public function updateCertStatus($status = 0)
    {
        if(!$this->_uid)
        {
            return false;
        ...
        $colunm[self::USER_CERT_STATUS] = (int) $status;
    ...
    {
        $key = $this->getUserDataKey($this->_uid);
        if($this->getRedis()->hExists($key,self::USER_CERT_STATUS) === false)
        {
            return -1;
        }

        return $this->getRedis()->hGet($key,self::USER_CERT_STATUS);
    }
    ...
    {
        $key = $this->getUserDataKey($this->_uid);
        if($this->getRedis()->hExists($key,self::USER_REAL_NAME) === false)
        {
            return -1;
        }

        $data = $this->getRedis()->hGet($key,self::USER_REAL_NAME);
     ...
     * @return [type]          [description]
     */
    public function updateCertZhimaStatus($status = 0)
    {
        if(!$this->_uid)
        {
            return false;
        ...
        $colunm[self::USER_CERT_ZHIMA] = (int) $status;
        return $this->_setUserColunm($this->_uid,$colunm);
    }
     ...
     * @return [type] [description]
     */
    public function getCertZhimaStatus()
    {
        $colunm[]   = self::USER_CERT_ZHIMA;
        return $this->_getUserColunm($this->_uid,$colunm);
    }

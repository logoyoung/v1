...
class HistoryService extends AbstractService
{
    ...
    public function getHistoryList()
    {
        ...
        if (!$historyList || !isset($historyList['list']) || empty($historyList['list']))
        {
        ...
        if (isset($historyList['list']) && !empty($historyList['list']))
        {
            ...
            foreach ($anchorInfos as $anchorInfo)
            {
                ...
                if (!$liveInfo)
                {
                    ...
                    $msg = self::$errorMsg[$code];
                    $log = "error_code:{$code};msg:{$msg};uid:{$anchorInfo['uid']}|class:" . __CLASS__ . ';func:' . __FUNCTION__ . ';line:' . __LINE__ . $this->getCaller();
                    write_log($log);
                } else
                {
                    if (LiveService::slaveIsLiving($anchorInfo['uid']) == LiveService::PLAY_TYPE_02)
                    {
                        $subPoster = LiveService::getSlaveDataByLiveId($liveInfo['liveid']);
                        $data['subPoster'] = isset($subPoster[$liveInfo['liveid']]['poster']) ? $subPoster[$liveInfo['liveid']]['poster'] : '';
                    } else
                    {                   
                        $data['subPoster'] = '';
                    }

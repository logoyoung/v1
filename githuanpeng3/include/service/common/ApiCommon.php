...
//        is_array($data) && array_walk_recursive($data, 'xss_clean');
    ...
    public function rander() {
        if ($this->resultFormatData['code'] == 0) {
            $result = $this->resultFormatData['data'];
            render_json($result, $this->resultFormatData['code'], 1);
        } else {
            $result = [
                'desc' => $this->resultFormatData['desc'],
                'code' => $this->resultFormatData['code'],
            ];
            render_json($result, $this->resultFormatData['code'], 2);
        }
    }

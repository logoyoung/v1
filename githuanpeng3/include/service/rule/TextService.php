...
use Exception;
use service\rule\helper\HuanPengWordFilter;
...
class TextService extends AbstractService
{
    ...
    public static $huanpengFilterChannel = [
        self::CHANNEL_THEME,
        self::CHANNEL_NICKNAME,
    ];
    ...
    private $_logErrorName  = 'shumei_access';
    ...
    public function checkStatus()
    {
        ...
        if($total == 1)
        {
            ...
            if(isset($result['is_huanpeng_filter']))
            {
                return $this->_checkHuanPengFilterReuslt($result);
            }
        ...
        foreach ($result as $key => $val)
        {
            ...
            if(isset($val['is_huanpeng_filter']))
            {
                $checkResult[$key] = $this->_checkHuanPengFilterReuslt($val);
            } else
            {
    ...
    public function getResult()
    {
        ...
        foreach ($result as $k => $r)
        {
            ...
            //欢朋内部过滤器
            if(in_array($this->_param[$k]->getChannel(), self::$huanpengFilterChannel))
            {
                $word = HuanPengWordFilter::checkFilterWord($this->_param[$k]->getText());
                if($word !== true)
                {
                    $this->_result[$k] = $this->_huanpengFilterParam(false,$this->_param[$k]->getText(),$word);
                    continue;
                }
    ...
    private function _huanpengFilterParam($status,$inputWord,$filterWord)
    {
        return [
            'is_huanpeng_filter'   => 1,
            'status'               => (bool) $status,
    ...
    private function _checkHuanPengFilterReuslt($result)
    {
        if($result['status'] !== true)
        {
            write_log("error|欢朋官方违禁词:{$result['filter_word']}; 原句:{$result['input_word']}; line:".__LINE__,$this->_logDebugName);
            return false;
        }

...
//增长规则
    ...
    public function OptionRoomViewNum( $luid )
    {
        ...
        }else
        {
            ...
            $viewer = $viewer + $this->getRoomViewerNumSpeedRule($viewerCount);
            ...
            if ( $md > 0 )
            {
                ...
                {
                    //按速率增长
                    $md = ceil($viewer/$this->_raiseSpeed)*self::SLEEP_SECOND;
                }
            ...
            else
            {
                ...
                {
                    //按速率减少
                    $md = ceil($md/$this->_reduceSpeed)*self::SLEEP_SECOND;
                }
    ...
    {
        $robotData = $this->getRobotData();
        $data = $robotData->getTotalRoomRobotViewer();
        if($data === false)
        {
            $log ="errror:mysql failed | error_msg: can't get getTotalRoomRobotViewer |class:" . __CLASS__ . ';func:' .  __FUNCTION__ .';line:' . __LINE__ ;
            $this->writelog($log);
            return false;
        }
        return $data[0]['total'];
    ...
    public function getTotalSearchViewerInfo($postData)
    {
        $robotData = $this->getRobotData();
        $data = $robotData->getTotalSearchViewerInfo($postData);
        if($data === false)
        {
            $log ="errror:mysql failed | error_msg: can't get selectRoomRobotViewer |class:" . __CLASS__ . ';func:' .  __FUNCTION__ .';line:' . __LINE__ ;
            $this->writelog($log);
    ...
    {
        $key = $GLOBALS['env'].self::TIMER_KEY.$roomId.'_'.$uid;
        $res = $this->getRedis()->get($key);
        if(!$res)
        {

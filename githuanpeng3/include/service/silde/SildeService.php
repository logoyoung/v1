...
class SildeService extends AbstractService
{
    ...
    public function getLiveListByGameId()
    {
        ...
        $liveListDataService->setSize($this->getSize());
        
        if($this->_params['gameId'] == LiveListDataService::DOUBLE_SCREEN_ID)
        {
            $liveInfos = $liveListDataService->getDoubleScreenLiveList();
        } else
        {
            $liveListDataService->setLiveType(LiveListDataService::LIVE_TYPE_HOT);
            $liveListDataService->setGameId($this->_params['gameId']);
            $liveInfos = $liveListDataService->getLiveListByLiveTypeAndGameId();
        }
    ...
    public function getIndexLiveList()
    {
        ...
        //将双屏直播id放入
        array_unshift($gameids, LiveListDataService::DOUBLE_SCREEN_ID);
        $floorGameLiveList = [];
        foreach ($gameids as $gameid)
        {
            $this->_params['gameId'] = $gameid;
            $res = $this->getLiveListByGameId();
            $floorGameLiveList = $res ?  array_merge($floorGameLiveList, $res) : $floorGameLiveList;
        }

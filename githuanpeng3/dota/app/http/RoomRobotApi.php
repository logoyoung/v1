...
class RoomRobotApi
{
    ...
    public function select()
    {
        ...
        $res['list'] = $list;
        $res['total'] = $roomRobotService->getTotalRoomRobotViewer();
        render_json($res);
    }
    #搜索
    ...
    {
        $this->_init();
        $checkData['uid'] = json_decode($this->_uid,true);
        $checkData['page'] = $this->_page;
        $checkData['pageSize'] = $this->_pageSize;
        //不能小于等于0
        $checkRule =['uid'=>'1','page'=>'2','pageSize'=>'2'] ;
        ApiCommon::checkParams($checkData,$checkRule);
        $roomRobotService = new RoomRobotService();
        ...
        $res['list'] = $list;
        $res['total'] = $roomRobotService->getTotalSearchViewerInfo($checkData);
        render_json($res);
    }
    #增
    public function add()

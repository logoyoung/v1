<?php
require __DIR__.'/../../include/init.php';
use service\room\RoomManagerService;

class test
{
    public function getRoomidByUids($uid = [1870])
    {
        $service = new RoomManagerService();
        $service->setUid($uid);
        $r = $service->getRoomidByUid();
        print_r($r);
    }

    public function getUidByRoomid($roomid = [100298,100342])
    {
        $service = new RoomManagerService();
        $service->setRoomid($roomid);
        $r = $service->getUidByRoomid();
        print_r($r);
    }

    public function getUidToRoomId($uid = 1870)
    {
        $service = new RoomManagerService();
        $service->setUid($uid);
        $r = $service->getUidToRoomId();
        var_dump($r);
    }

    public function getManagerList($uid = 1870)
    {
        $service = new RoomManagerService();
        $service->setUid($uid);
        $r = $service->getRoomManagerList();
        var_dump($r);
    }

    public function addManager($anchorUid=2335,$managerUid=47420)
    {
        $service = new RoomManagerService();
        $service->setUid($anchorUid);
        $service->setManagerUid($managerUid);
        $s = $service->addRoomManager();
        var_dump($s);
    }

    public function deleteManager($anchorUid=2335,$managerUid=47420)
    {
        $service = new RoomManagerService();
        $service->setUid($anchorUid);
        $service->setManagerUid($managerUid);
        $s = $service->deleteRoomManager();
        var_dump($s);
    }
}

$obj = new test;
//$obj->deleteManager();
//die;
$obj->addManager();
die;
$obj->getManagerList($uid = 1870);
die;
$obj->getUidToRoomId($uid = 1870);
die;
$obj->getRoomidByUids([12350,12170]);
$obj->getUidByRoomid([100402,100401]);
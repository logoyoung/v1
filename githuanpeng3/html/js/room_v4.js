...
function rm_follow() {
    ...
    if (is_rm_follow_loading || !check_login()) {
        return;
    }
...
function follow_room() {
    ...
    if (is_follow_room_loading || !check_login()) {
        return;
    }
...
! function(a) {
    ...
    function treasure_receive() {
        ...
        if (treasure_loading || !curr_trid || !check_login()) {
            return;
        }
...
! function(a) {
    ...
    chat_obj.sendGift = function(c) {
        ...
        var content = { 't': 103, 'enc': getCookie('_enc'), 'gid': c.giftid, 'liveid': $ROOM.liveID, 'identity': myGroupID, 'sendType': c.sendType };
...
window.proxyCallBack = function(a, b) {
    ...
    //602 双屏 关流
    if (JSON.parse(b).t == '602' && JSON.parse(b).liveType == 4) {
        return;
    }
    //console.log(obj[a][j]);
...
! function(a) {
    ...
    room.push_phone = function() {
        ...
        $('#qrCode').qrcode({ render: 'canvas', text: $conf.domain + 'mobile/room/room.html?luid=' + $ROOM.anchorUserID, width: 110, height: 110 });
    ...
    room.gift = function() {
        ...
        $('.gift li .lw_item').bind('click', function() {
            ...
            //新增 礼物 类型
            var sendType = $(this).parent().data('sendType') || 0;
            if (gifttype == 2) {
                if (!check_login()) {
                    return false;
                }
    ...
    room.initSwfObbject = function() {
        ...
        var liveStatus = $ROOM.isLiving;

        var baseSWFObjectConf = {
            flashVersion: '9.0.0',
            install: 'expressInstall.swf?v=1.0.5',
            width: '100',
            height: '100',
            flashVar: {},
            params: {},
        ...
        var embedSwfObject = [{
                id: 'imProxy',
                file: $conf.domain + 'static/chatProxy.swf?v=1.0.5',
                width: '100',
                height: '100',
                params: {
                    quality: 'high',
            ...
            {
                id: 'rtmpplayer_room',
                file: $conf.domain + 'static/flash/rtmpplayer_js.swf?v=1.0.5',
                width: '100%',
                height: '100%',
                flashVar: {
                    'urlb': $conf.domain + 'static/flash/barrage.swf?v=1.0.5',
                    'urlw': $conf.domain + 'static/flash/wait.swf?v=1.0.5',
                    'urld': $conf.domain + 'static/flash/dot.swf?v=1.0.5',
                    'loadingURL': $conf.domain + 'static/flash/loading.swf?v=1.0.5',
                    'UIButtonURL': $conf.domain + 'static/flash/UIButton.swf?v=1.0.5',
                    'giftURL': $conf.domain + 'static/flash/gift.swf?v=1.0.5',
                    'maxCharacterNumber': 50,
                    'minTimeInterval': 2,
                    'UID': pageUser.isLogin ? pageUser.user.userID : 0,
                    'hostID': anchorUserID ? anchorUserID : 0,
                    'isLiving': liveStatus ? liveStatus : 0,
                    "recommendPHP": $conf.api + 'other/flashRecommend.php',
                    'urlPHP': $conf.api + 'live/getStreamList.php',
                    "LiveRecommendURL": $conf.domain + 'static/flash/LiveRecommend.swf?v=1.0.5',
            ...
            {
                id: 'getVoiceDevice',
                file: $conf.domain + 'static/flash/symvm.swf?v=1.0.5'
            }
        ];
    ...
    room.initAnchorLiveStatus = function() {
    ...
    room.initWatchInfo = function() {
        //统计时长,传给flash 值
        var watchUID = getCookie('_uid') || sessionStorage.getItem('_uid') || '';
        var watchENC = getCookie('_enc') || sessionStorage.getItem('_enc') || '';

        if (watchUID == '' || watchENC == '') {
            return;
        }
        setTimeout(function() {
            //延迟 3秒,传递个人信息
            runSwfFunction('rtmpplayer_room', 'setUserInfo', parseInt(watchUID), watchENC);
        }, 3000);
    };

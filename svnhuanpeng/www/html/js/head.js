...
var API_MAP = {
	...
	_dataForm:function (data,filed) {
		var t = [];
		if(data != ''){
            var ret = [].map.call(data,function (x){
                for(var p in filed){
                    x[p] = x[filed[p]];
                    //delete x[filed[p]];
                }
	...
	_getHistoryList:function(size){
		...
		$.ajax({
			...
			success:function(data){
				if(data.content.total == 0 || data.content == '' || !data.content.list){
					var domUlStr = '<li class="more no_login">'
	...
	_getFollowList:function(size){
		...
		$.ajax({
			...
			success:function(data){
				...
				if(!data.content.liveTotal || !data.content.total || data.content == ''){
                    var domUlStr = '<div class="livecount">当前关注的有<em style="color:#ff7800;margin:0px 3px;font-style:normal">'+data.liveTotal+'</em>个正在直播</div>';
                    domUlStr = '<li class="more no_login">'
                        + '<div class="img_no_login">'
                        + '<img src="'+o._conf['domain']+'static/img/logo/home_no_login.png">'
	...
	_getUserInfo:function(){
		...
		$.ajax({
			...
			success:function(data){console.log(data)
				...
				var u = data;
				//var $conf = this._conf;//alert(u.integral+'--'+u.levelIntegral);
				var finished = (u.integral / u.levelIntegral) * 100 + '%';
		        var c = u.levelIntegral - u.integral;
		        //if(c==0) c=20;
		        ...
		        htmlstr +='<div class="clear"></div>';
		        htmlstr +='<span class="levelup">距离升级还有<a>'+ digitsFormat(data.gapIntegral) +'</a>经验值</span>';
		        htmlstr +='</div>';

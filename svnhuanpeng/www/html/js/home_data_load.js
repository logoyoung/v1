...
hd.prototype={
		...
		_loadYouLike:function(data){
			...
			var o = arguments[1];
			//var data = data[0];
		   /* for(var key in guessLoveList){
		      	guessLoveList[key].posterUrl = guessLoveList[key].posterUrl?guessLoveList[key].posterUrl:o._tmpPic; 
		           //console.log(guessLoveList[key]);
		    		var guessImg = guessLove[key].getElementsByTagName('img');
		   	    //if(guessLoveList[key].posterUrl)
		   	    $(guessImg).attr('src',guessLoveList[key].posterUrl);console.log(guessLoveList[key].posterUrl);
		   	    var anchorName = guessLove[key].getElementsByClassName('anchor');
		   	    $(anchorName).text(guessLoveList[key].nick);
		   	    var liveTitle = guessLove[key].getElementsByTagName('p');
		   	    $(liveTitle).text(guessLoveList[key].liveTitle);
		   	    var spanObj = guessLove[key].getElementsByTagName('span');
		   	    $(spanObj[0]).text(guessLoveList[key].viewerCount);
		      	$(spanObj[1]).text(guessLoveList[key].fansCount);
		        	$(spanObj[2]).text(guessLoveList[key].gameName);
		        	var link = guessLove[key].getElementsByTagName('a');
		        	$(link).attr('href','http://dev.huanpeng.com/main/liveRoom.php?luid='+guessLoveList[key].luid);
		    }*/
			var domUlStr = '';
			var domLiStr = '';
			var ulObj = $('#block1 ul');
		...
		_gameList:function(gameIds){
			...
			//url = 'http://dev.huanpeng.com/main/a/gameInfoList.php';

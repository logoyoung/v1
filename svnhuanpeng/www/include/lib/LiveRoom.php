...
//	}
	...
	public function sendGift( int $uid, array $params )
	{
		...
		if ( !$errno )
		{
			if ( !Gift::addGiftRecord( $otid, $uid, $this->_luid, $liveID, $giftID, $sendNum, $type, $this->_db ) )
			{
				$errno = -3510;
			}
			else
		...
		if ( !$errno )
		{
			...
			if ( $isSendBean )
			{
				...
				{
					//TODO log 异常错误
					$this->_log( "sendGiftSuccessCallBack($otid, $type,{$result['tid']}, {$result['ctime']})" );
				}
			}
			else
			{
			...
			if ( $isSendBean )
			{
				$userObject->updateUserHpBean( $result['shd'] );
				$this->_anchorObj->updateAnchorBean( $result['rgd'] );
			...
			{
				$userObject->updateUserHpCoin( $result['shb'] );
				$this->_anchorObj->updateAnchorCoin( $result['rgb'] );
				Gift::askRedisIfSendGiftSuccess( $uid, $this->_luid, $type, $result['shbd'], $result['rgbd'] );
				$this->_rankObj->intoRankList( $uid, $cost );
...
//		$event->trigger($event::ACTION_USER_MONEY_UPDATE,['uid' => $this->_luid]);
...
//					$treasureID        = $this->_createRoomTreasure( $uid );
	 ...
	 * @return bool
	 */
	public function stop( $liveid, $reasonid = 0, $reason = '' )
	{
		//TODO: stop
		$msgPackage = MsgPackage::getLiveEndMsgSocketPackage( $this->_luid, $liveid, $reasonid, $reason );
		$this->_sendSocketMsg( $msgPackage );
...
//		$this->_endLiveStatistic( $liveid );
	...
	public function isShowWel( $level )
	{

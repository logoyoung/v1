...
//            'ip' => '122.70.146.49',
    ...
    public function __construct($host = '172.20.28.147', $port = 9981) {
		//todo 链接管理，需要使用配置数组来做，方便多redis 服务器切换以及负载均衡
        $this->env = $GLOBALS['env'];
        $addr = $this->redisConf[$this->env];
        $this->redis = new Redis();
        $this->redis->connect($addr['ip'], (int)$addr['port']);
        return $this->redis;
    }

//    private function __construct() { //私有的构造方法
//        self::$redis = new Redis();
//        self::$redis->connect($host = '172.20.28.147', $port = 9981);
//    }
//
//    //获取静态实例
...
//    }
   ...
   */
     ...
     * @param string $setName 集合名字
     */
    public function sismember($setName,$setValue) {
        return $this->redis->sismember($setName,$setValue);
    }

...
class IndexService extends AbstractService
{
    ...
    public function getRecommendInformationListData($type, $client)
    {
        ...
        $res = $baseInfoData->getRecommemdInfo($type, $client);
        if ($res)
        {
            $conf = $GLOBALS['env-def'][$GLOBALS['env']];

            $Itype = $baseInfoData->getInformationType();
            if ($type == BaseInfo::INFORMATION_TYPE_All)
            {
                if (isset($res[1]))
                {
                    $plist = [];
                    $pres = $baseInfoData->getInfoListByIds($res[1]);
                    if ($pres)
                    {
                        ...
                        for ($i = 0, $k = count($parray); $i < $k; $i++)
                        {
                            ...
                            $temp['poster'] = $pres[$parray[$i]]['poster'] ? DOMAIN_PROTOCOL . $conf['domain-img'] . '/' . $pres[$parray[$i]]['poster'] : '';
                ...
                {
                    $tlist = [];
                    $tres = $baseInfoData->getInfoListByIds($res[2]);
                    if ($tres)
                    {
            ...
            {
                $list = [];
                $pres = $baseInfoData->getInfoListByIds($res[$type]);
                if ($pres)
                {
                    ...
                    for ($i = 0, $k = count($parray); $i < $k; $i++)
                    {
                        ...
                        $temp['url'] = $pres[$parray[$i]]['url'];
                        $temp['isLogIn'] = $pres[$parray[$i]]['is_login'];
                        if ($type == BaseInfo::INFORMATION_TYPE_LIST)
                        {
                ...
                if ($type == BaseInfo::INFORMATION_TYPE_PICTUR)
                {//
                    return array('plist' => $list, 'tlist' => []);
                }
                if ($type == BaseInfo::INFORMATION_TYPE_LIST)
                {
                    return array('plist' => [], 'tlist' => $list);
                }
            }
        } else
        {
            return array('plist' => [], 'tlist' => []);
        }
    }
    ...
    public function getHomePageGameList()
    {
        $data = [];
        $liveTypeKey = [
            GameService::LIVE_TYPE_HOT => 'hot',
            GameService::LIVE_TYPE_NEW => 'new',
            GameService::LIVE_TYPE_FOLLOW => 'maxfollow'
        ];
        ...
        if (!$data)
        {
            ...
            foreach (self::$gameIds as $game => $gameId)
            {
                foreach (self::$liveTypeAll as $liveType)
                {
                    $data[$game][$liveTypeKey[$liveType]] = $gameService->getLiveListByGameId($liveType, self::DEFAULT_PAGE, self::INDEX_GAME_LIVE_PC_NUM, $gameId);

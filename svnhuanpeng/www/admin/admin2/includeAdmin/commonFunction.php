 ...
 * @return bool
 */
function addRoleChangeRecord( $data, $db )
{
	if( empty( $data ) )
	{
		return false;
...
function getBeforeRateByUid( $uid, $db )
{
	if( empty( $uid ) )
	{
		return false;
	}
	$res = $db->field( 'cid,rate' )->where( "uid=$uid" )->limit( 1 )->select( 'anchor' );
	if( false !== $res )
	{
		return $res[0];
	}
 ...
 * @return bool
 */
function updateNoticStatus( $rate_change_id, $db )
{
 ...
 * @param $db
 */
function  unsuccessLogForFinanceBack($title,$desc,$db){
	$data = array(
		'title' => $title,

...
//Array.prototype.deletes = function(index){
//    if(index < 0)
//        return this;
//    else
        ...
        init: function (d, c) {
            return !function () {
                ...
                var interval = setInterval(function () {
                    ...
                    j++;
                    j = j == c.num ? 0 : j;
                    //console.log(d.css('background-position'));
                }, timer);
            }();
        }
    };
    ...
    Sidebar = {
        ...
        initFollowList: function () {
            ...
            (function initFollowListInfo() {
                ...
                if (_isLogin)
                    f.hover(loginEvent, function () {
                        return false;
                    });
                else
                    f.hover(nologinEvent, function () {
                        return false;
                    });

                function nologinEvent() {
                    var htmlstr = '<div class="followOne no-data"><span class="no-data-logo"><img src="/static/img/logo/home_no_login.png" alt=""/></span>'
                    + '<div style="line-height:46px;">还没登录？点击<div class="to_reg" style="color:#ff7800;float:right;">注册</div><div style="float:right;width:1px;height:13px;background:#999;'
                    + 'margin:16px 5px 5px 5px;"></div><div class="to_log" style="color:#ff7800;float:right;">登录</div></div></div>';
                    a("#followList .followList_detail").html(htmlstr);
                    a('#followList .followOne:last').css('border-bottom', '0');
                    a('#followList .viewall').css('display', 'none');
                    a('.to_log').click(function () {
                        loginFast.login(0);
                    });
                    a('.to_reg').click(function () {
                        loginFast.login(1);
                    });
                };
                ...
                function loginEvent() {
                    ...
                    var loadStr = '<div id="followload-animate">数据加载中.....</div>';
                    a("#followList .followList_detail").html(loadStr);
                    loadAnimate.selector('followload-animate').showLoad();
                    var requestData = {
                        uid:uid,
                        encpass:enc,
                        size:3
                    ...
                    ajaxRequest({url:requestUrl, data:requestData}, function(d){
                        ...
                        // var htmlstr = '';
                        htmlstr += '<div style="float:left;margin:12px 12px 0px 12px;">当前关注的有<em style="color:#ff7800;margin:0px 3px;font-style:normal">' + d.liveTotal + '</em>个正在直播</div>';
                        for (var i in flist) {
                            //var href = $conf.domain + 'room.php?luid=' + flist[i].uid;
                            var href = $conf.domain + flist[i].roomID;
                            htmlstr += '<a href="' + href + '">';
                            htmlstr += '<div class="followOne">';
                            htmlstr += '    <div class="f_face">';
                            htmlstr += '       <img src="' + flist[i].head + '">';
                            htmlstr += '    </div>';
                            htmlstr += '    <div class="f_info">'
                            htmlstr += '        <p>' + flist[i].nick + '</p>';
                            htmlstr += '        <div class="clear"></div>';
                            htmlstr += flist[i]['isLiving'] == '1' ? '<span class="playtime">' + initPlaytimeHtml(flist[i].stime) + '</span>' : '<span class="playtime">暂未直播</span>';
                            htmlstr += "<div class='right'>";
                            htmlstr += '        <span class="anchor_icon viewerIcon2"></span>';
                            htmlstr += '        <span class="viewercount">' + flist[i].viewCount + '</span>';
                            htmlstr += "</div>";
                            htmlstr += '    </div>';
                            htmlstr += ' </div>';
                            htmlstr += '</a>';
        ...
        initHistoryList: function () {
            ...
            (function () {
                ...
                if (_isLogin)
                    h.hover(loginEvent, function () {
                        return false;
                    });
                else
                    h.hover(nologinEvent, function () {
                        return false;
                    });

                function nologinEvent() {
                    var htmlstr = '<div class="historyOne no-data"><span class="no-data-logo"><img src="/static/img/logo/home_no_login.png" alt=""/></span>'
                    + '<div style="line-height:46px;">还没登录？点击<div class="to_reg" style="color:#ff7800;float:right;">注册</div><div style="float:right;width:1px;height:13px;background:#999;'
                    + 'margin:16px 5px 5px 5px;"></div><div class="to_log" style="color:#ff7800;float:right;">登录</div></div></div>';
                    a("#historyList .historyList_detail").html(htmlstr);
                    a('#historyList .historyOne:last').css('border-bottom', '0');
                    a('#historyList .viewall').css('display', 'none');
                    a('.to_log').click(function () {
                        loginFast.login(0);
                    });
                    a('.to_reg').click(function () {
                ...
                function loginEvent() {
                    var uid = getCookie('_uid');
                    var enc = getCookie('_enc');
                    var loadStr = '<div id="historyload-animate">数据加载中.....</div>';
                    a("#historyList .historyList_detail").html(loadStr);
                    loadAnimate.selector('historyload-animate').showLoad();
                    ...
                    ajaxRequest({url:requestUrl, data:requestData,load:function(){
                        ...
                        for(var i in hlist){
                            //var href = $conf.domain + '/room.php?luid=' + hlist[i].uid;
                            var href = $conf.domain + hlist[i].roomID;
                            htmlstr += '<a href="' + href + '">';
                            htmlstr += '<div class="historyOne">';
                            htmlstr += '<div class="h_face">';
                            htmlstr += '<img src="' + hlist[i].head + '">';
                            htmlstr += '</div>';
                            ...
                            htmlstr += '</span>';
                            htmlstr += '<p>' + hlist[i].nick + '</p>';
                            htmlstr += '<div class="clear"></div>';
                            htmlstr += '<span class="anchor_icon viewTimeIcon"></span>';
                            htmlstr += '<span class="playtime">' + calVisitTime(hlist[i].stime) + '</span>';
                            htmlstr += '</div>';
                            htmlstr += '</div>';
                            htmlstr += '</a>';
                        }

                        htmlstr = htmlstr != '' ? htmlstr : '<div class="historyOne no-data"><span class="no-data-logo"><img src="'+$conf.domain+'static/img/logo/home_no_login.png" alt=""/></span><span class="no-data-title">您还没有浏览历史哦</span></div>';;
                        a("#historyList .historyList_detail").html(htmlstr);
                        a('#historyList .historyOne:last').css('border-bottom', '0');
                    },function(){
        ...
        initGameList: function () {
            ...
            var requestUrl = $conf.api + 'game/gameList.php';
...
var interrupt = [];
var msgToClient = {};
var needUpdateVersion = $ROOM.client.needUpdateVersion;
function isNeedUpdate( cur, need )
{
    if( cur.length > need.length ){
        return false;
    }
    if( cur.length > need.length )
    {
...
!function () {
    ...
    var orderList = ['connect', 'start', 'run', null, 'closeClient', 'reset'];
    ...
    var lock_setLiveStart = false;//更改直播状态请求锁
    ...
    function returnHttpLiveInfo() {
        var httpData = msgToClient.httpData;
        return {
            stream: httpData.stream,
            server: httpData.server,
            quality: httpData.quality,
            //title: httpData.title,
            //gameName: httpData.gameName,
            audio: httpData.audio,
            bitrate:$ROOM.client.bitrate,
            width:$ROOM.client.width,
            height:$ROOM.client.height
        };
    }

    function returnStepHttpData(order, port) {
        //console.log(order);
        order = orderList.indexOf(order);
        var msgData = {
            0: {port: port},
            1: {uid: getCookie('_uid'), encpass: getCookie('_enc')},
            2: {isLiving: msgToClient.httpData.isLiving},
            4: {},
            5: {}
        };
        for (var i in msgData) {
            msgData[i] = $.extend({}, {order: order}, msgData[i]);
        }
        msgData[1] = $.extend({}, msgData[1], returnHttpLiveInfo());

        //console.log(msgData[order]);
        return {
            url: local + ':' + port + file,
            data: msgData[order]
        }
    ...
    function connect() {
        ...
        for (var i in portList) {
            msgToClient.sendMsg(returnStepHttpData('connect', portList[i]));

        }
    }

    function liveStartSuccHandle() {
        //console.log('liveStartSuccHandle');
        msgToClient.httpData.isLiving = 1;
        anchor_is_on_living = 1;
        status = 2;
        setCookie('hostLiveID', $ROOM.liveID);
        //防止未接收到socket消息
        !socket_liveStart_receive && (socket_liveStart_receive = false) && initPlayer('rtmpplayer_room',$ROOM.anchorUserID);
        initStopLiveBtn();
    }

    function liveStartFailedHandle() {
        tips('直播开启失败');
        msgToClient.order = 4;
    }

    function requestSetLive(continueLive) {
        var url = $conf.api + 'client/';
        url += continueLive ? 'liveContinue.php' : 'liveStart.php';

        if (lock_setLiveStart) return;
        lock_setLiveStart = true;
        var requestData = {
            uid:getCookie('_uid'),
            encpass:getCookie('_enc'),
            liveID:$ROOM.liveID
        };
        ajaxRequest({url:url,data:requestData},function(d){
            if(continueLive) msgToClient.continueLive = 0;
            loading('',true);
            liveStartSuccHandle();
        },function(){
            if(continueLive) msgToClient.continueLive = 0;
            loading('',true);
            liveStartFailedHandle();
        });
    }

    function addInterruptBlock(clientStat) {
        interruptBlock.time = new Date().getTime();
        interruptBlock.order = msgToClient.order;
        interruptBlock.status = status;
        interruptBlock.clientStat = clientStat;
        interruptBlock.requestData = msgToClient.httpData;
        interrupt.push(interruptBlock);
    }

    function popInterruptBlock() {
        if (interrupt.length >= 1) {
            var block = interrupt.pop();
            msgToClient.order = block.order;
            status = block.status;
            return block.requestData;
        }
        return false;
    }
        ...
        self.interval = setInterval(function () {
            if (timer >= timeOut) {
                ...
                if (self.order === 0) {
                    ...
                    self.sendMsg(returnStepHttpData(orderList[self.order], port));
                }
            }
            timer++;
        }, 1000);
    ...
    msgToClient.order = -1;
    msgToClient.httpData = {isLiving: 0, stream: '', server: '', title: '', quality: '', gameName: '', audio: ''};
    ...
    msgToClient.run = function (d) {
        ...
        self.bitrate = $ROOM.client.bitrate;
        self.width = $ROOM.client.width;
        self.height = $ROOM.client.height;
        self.httpData.quality = d.quality;
        self.httpData.audio = d.audio;
        setCookie('_uAudio', d.audio);
        self.init();
    };
    ...
    msgToClient.callBackFunc = function (d) {
        ...
        if( isNeedUpdate(d.build, needUpdateVersion ) )
        {
            port = port? port : res.port;
            status = 4;
            self.order = 5;
            tips('发布直播需要下载最新版客户端。。','', 5000);
            self.resetStatus();
            clearInterval( self.interval );
            return;
        }

        if(error.stopcode == 2008){
            status = 4;
            self.order = 5;
            //TODO continue live
            setTimeout(function(){pushLiveBox.startLive.init2();},2000);
            return;
        }
        if(status == 2 && error.stopcode == 2009){
            status = 4;
            self.order = 5;
            loading('与客户端连接中断....');
            return;
        ...
        if (error.code != 0) {
            ...
            if (error.code == 38) {
                ...
                if (self.order == 0 && status == -1) {
                    addInterruptBlock(clientStat);
                    if (res.port)port = res.port;
                ...
                if (status == 2 && clientStat == 4) {
                    status = 4;
                    pushLiveBox.stopLive.stopNotify();
                    self.order = 5;
                }
                if(status == 4 && clientStat == 4 && self.order==0){
                    self.order = 5;
                }
            }
        } else {
            ...
            if (clientStat == 1 && status == 1) {
                loading('点击客户端‘开始直播’按钮，即可开始直播', false, '该过程中请勿刷新页面');
            }
            if (clientStat == 2) {
                if (status == 1) {
                    requestSetLive(msgToClient.continueLive);
                } else if (status == 2) {
                    lock_setLiveStart && (lock_setLiveStart = false);
                }
                self.order = 2;
            }
            //客户端停止直播
            ...
            if ((clientStat == 4 || clientStat == 3) && self.order == 4 && self.httpData.isLiving == 1) {
                pushLiveBox.stopLive.stopNotify();
                self.order = 5;
            }
            //reset
    ...
    msgToClient.resetStatus = function () {
        ...
        timer = 0;
        port = 0;
        self.httpData = {isLiving: 0, stream: '', server: '', title: '', quality: '', gameName: '', audio: '',width:$ROOM.client.width,height:$ROOM.client.height,bitrate:$ROOM.client.bitrate};
        initPushLiveBtn();
    };
}();
...
window.getDeviceListCallBack = function ( type, list ) {
    ...
    console.log(dom.find('.options .myOption').eq(itemIndex));
    Select('.mySelect');

    pushLiveBox.pushLive.showModal.showModal();
}

//!function () {
//    var id = getDeviceSWFID;
//    var flashVersion = '9.0.0';
//    var install = 'expressInstall.swf';
//    var file = $conf.domain + 'static/flash/symvm.swf';
//    swfobject.embedSWF(file, id, '1px', '1px', flashVersion, install);
//    initDeviceList();
//}();

// function initDeviceList() {
//     var getDeviceSWFID = "rtmpplayer_room";
//     runSwfFunction(getDeviceSWFID, "getCM", "getDeviceListCallBack");
// }
...
function initDeviceList()
{
    ...
    var portList = [7613,7614,7615,7616,7617,7618,7619,7620,7621,7622];
    var requestTimer = 1;
    var requestLimit = portList.length;
    function unFindHpClient(){
        if(requestTimer >= requestLimit )
        {   console.log('---'+requestTimer+'----'+requestLimit);
            if(allowUseObs)
            {
                alert('未检测到您当前电脑上安装欢朋客户端，无法开启直播');
                return false;
            }

            getDeviceListCallBack( 'audio', [] );
        }
        requestTimer ++;
    }

    for(var i=0; i<portList.length;i++){
        var url = ip + ':' + portList[i] + '/getaudio'
        $.ajax({
            url:url,
            dataType:'jsonp',
            jsonp:'jsoncallback',
            ...
            success:function (responseData) {
                ...
                if( responseData.device && responseData.device.num > 0 )
                {
                    getDeviceListCallBack( 'audio', responseData.device.list );
                }else{
                    unFindHpClient();
                }
            },
        ...
        $($('head script').get()[0]).error(function () {
            var src = $(this).attr('src');

            if( src && /getaudio/.test( src )){
                unFindHpClient();
            }
        });
    }
...
!function (a) {
    var gameList = {};
    gameList.all = $ROOM['gameList'];//["梦幻西游", "炉石传说", "FIFA 15", "太极熊猫", "乱斗西游", "全民奇迹", "我叫MT2", "狂野飙车8", "Real Racing 3", "Temple Run 2", "天天酷跑", "混沌与秩序对决", "刀塔传奇", "现代战争5", "Dead Trigger 2", "海岛奇兵", "部落冲突", "开心消消乐", "糖果传奇", "欢乐斗地主", "网球精英 3", "刀塔西游", "皇室战争", "火影忍者", "梦三国", "球球大作战", "全名超神", "全名枪战", "热血传奇", "时空召唤", "天天飞车", "天天炫斗", "王牌对决", "王者荣耀", "我的世界", "侠盗猎车手", "虚荣", "自由之战", "CF枪战王者"];//pageUser.gameHistoryList || [];
    gameList.hot = ["梦幻西游", "炉石传说", "FIFA 15", "太极熊猫", "乱斗西游"];//$ROOM.hotGameList || [];
    gameList.history = $ROOM['gameHistory'];//["Real Racing 3", "Temple Run 2", "天天酷跑", "混沌与秩序对决", "刀塔传奇"];//$ROOM.allGame || [];
    ...
    function stopLiveRequest(callBack) {
        var requestUrl = $conf.api + 'live/stopLive.php';
        var requestData = {
            uid:anchorObj.uid,
    ...
    function openTheClient() {
        var id = "open_the_client";
        if ($('#' + id).get()[0]) {
            $('#' + id).remove();
        }

        var html = '<div id="'+id+'" style="display:none;"><iframe src="HuanpengTV://" style="display:none;"></iframe></div>';
        $('body').append(html);
    }
    ...
    pushLiveBox.startLive = {
        init: function () {
            ...
            var requestUrl = $conf.api + 'live/isLiving.php';
            ajaxRequest({url:requestUrl, data:requestData},function (d) {
                var stat = d.status;
                if(stat == 1){
                    pushLiveBox.pushLive.init(d.list);
                }else if(stat == 0){
        ...
        init2:function(){
            ...
            var requestUrl = $conf.api + 'live/isLiving.php';
            ajaxRequest({url:requestUrl,data:requestData},function(d){
               var stat = parseInt(d.status);
               if(stat == 0){
                   $ROOM.liveID = d.liveID;
                   self.continueLive(d.list,'您当前网络情况不好，客户端已经停止，是否要继续直播');
               }
            },function(){
        ...
        continueLive: function (data) {
            ...
            var noticeTitle = arguments[1] ? arguments[1] : '您现在正在直播，是否继续直播';
            var diaLog = dialog({
                skin: 'pushBox box-content err-notice box-notice',
                title: '提示',
                content: '<p>'+noticeTitle+'</p>',
                cancelValue: '取消',
                cancel: function () {
                ...
                okValue: '继续直播',
                ok: function () {
                    var continueAudio = getCookie('_uAudio') ? getCookie('_uAudio') : '';
                    openTheClient();
                    ...
                    var requestUrl = $conf.api + 'live/continueLiving.php';
                    ajaxRequest({url:requestUrl,data:requestData},function(d){
                        msgToClient.continueLive = true;
                        msgToClient.run({
                            title: data.title,
                            gameName: data.gamename,
    ...
    pushLiveBox.pushLive = {
        ...
        init: function (d) {
            ...
            boxModal.find('.ui-dialog-footer').append('<a style="margin-left: 20px; color: #0064b4; float: left; margin-top: 11px;position: absolute;z-index: 2" href="'+$conf.domain +'help/helpAssistant.php">直播教程</a><div class="clear"></div>').find('.ui-dialog-button').append('<div class="clear"></div>');
            boxModal.find('.ui-dialog-footer').append('<div class="insert-image"></div>');
            if (gameList.history.length) {
                boxModal.find('#history-game .game-list li:eq(0)').addClass('selected');
                boxModal.find('#input-gameName').val(gameList.history[0]);
            } else {
                boxModal.find('#hot-game .game-list li:eq(0)').addClass('selected');
                boxModal.find('#input-gameName').val(gameList.hot[0]);
            }
            self.initGameSelectEvent();
            initDeviceList();
            //Select('.mySelect');
            $('.select-quality .checkboxDiv').bind('click', function () {
                $('.select-quality .checkboxDiv .checkbox-label').removeClass('checked');
                $(this).find('.checkbox-label').addClass('checked');
            });
            var title = d.title ? d.title : getCookie('_unick') + '的直播间';
            var qualityselect = d.quality == 0 ? 0 : 1;
            $('#input-liveTitle').val(title);
            if (qualityselect) {
                $('.select-quality .checkboxDiv').find('.checkbox-label').removeClass('.selected').eq(0).addClass('selected');
            } else {
                $('.select-quality .checkboxDiv').find('.checkbox-label').removeClass('.selected').eq(1).addClass('selected');
            }
            self.shareInit();
        },
        liveStart: function () {
            ...
            var quality = $('.select-quality .checkbox-label.checked input').val() ? $('.select-quality .checkbox-label.checked input').val() : 'high';
            var audio = $("#audio-value").val();
            var orientation = 1;
            var requestUrl = $conf.api + 'client/createLive.php';
            var requestData = {
                uid:anchorObj.uid,
                encpass:anchorObj.enc,
                deviceID:anchorObj.enc,
            ...
            ajaxRequest({url:requestUrl,data:requestData},function (d) {
                if(d.liveID){
                    openTheClient();
                    $ROOM.liveID = d.liveID;
                    initUserLiveInfo({title:title,gameName:game});
                    msgToClient.run({
                        // title:title,
                        // gameName:game,
                        quality:quality,
        ...
        shareInit:function(){
            //var shareUrl = $conf.domain + 'sharer.php?luid=' + $ROOM.anchorUserID;
            var shareUrl = $conf.domain + 'sharer.php?luid=' + $ROOM.anchorUserID;
            var uid = pageUser.user.userID;
            var cmdData = {
                tsina:'weibo',
                tqq:'qq',
                tqzone:'qq',
            ...
            $('.pushBox .share-channel .shareToWx .qrcode').qrcode({
                render:'canvas',
                text:shareUrl,
                width:120,
                height:120
            ...
            $('.pushBox .share-channel li').click(function () {
                var title = $('#input-liveTitle').val() ? $('#input-liveTitle').val() : $ROOM.anchorNickName +'的直播间';
                var cmd = $(this).data('cmd');
                if(cmd == 'wx'){
                    return;
                }else{
                    var shareDesc = getShareContent(cmdData[cmd], title, $ROOM.anchorNickName, $ROOM.anchorUserID,uid);
                    var option = {
                        url:shareUrl,
                        sumary:'我在#欢朋直播#,【'+$ROOM.gameName+'】等待你的观看~还在犹豫什么,一起策马奔腾吧',
    ...
    pushLiveBox.stopLive = {
        ...
        stopNotify: function () {
            var self = this;
            self.stopBox && self.stopBox.close().remove();
            var requestUrl = $conf.api+'live/stopLive.php';
            var requestData = {
                uid:anchorObj.uid,
                encpass:anchorObj.enc,
            ...
            function requestMyLiveInfo() {
                var requestUrl = $conf.api+'live/endLiveForApp';
                var requestData  = {
                    uid:anchorObj.uid,
                    encpass:anchorObj.enc,
                    liveID:$ROOM.liveID
                };
                ajaxRequest({ur:requestUrl,data:requestData},function (d) {
                    var content = pushLiveBox.content.stopNotify(d) + pushLiveBox.content.share + '<div class="clear"></div>';
                    var diaLog = dialog({
                        skin:'pushBox box-content error-notice live-stop-notify',
                        title:'结束直播',
                        content:content,
                        okValue: '完成',
                        ok: function () {
                            if (!d.autoFull || !$('#isAutoPublish').is(':checked')) {
                                requestAutoPublish();
                            }
                            self.share();
                        }
                    ...
                    $('.pushBox.box-content .ui-dialog-footer').append('<div class="insert-image"></div>');
                },function(d){
            ...
            function requestAutoPublish() {
                var requestUrl = $conf.api + 'live/autokeepEndLive.php';
                var requestData = {
                    uid:anchorObj.uid,
                    encpass:anchorObj.enc,
                    liveID:$ROOM.liveID,
                ...
                ajaxRequest({url:requestUrl,data:requestData});
            }
        },
        share: function () {
    ...
    pushLiveBox.content = {
        share: '<div class="dialog-right share-to-friend"> <p class="share-title">通知好友前来围观</p> <div class="share-channel"> <ul> <li class="share-weibo" data-cmd="tsina"></li> <li class="share-qq" data-cmd="tqq"></li> <li class="share-qzone" data-cmd="tqzone"></li> <li class="share-weixin" data-cmd="wx"><div class="shareToWx none"> <div class="qrcode"> <img src="'+$conf.domain+'static/img/src/qrcode/qrcode_home.png" alt=""/> </div> <span class="share_icon wx-icon"></span> </div></li> </ul> </div> </div>',
        pushContent: function () {
            return '<div class="dialog-left">' + return_liveTitle_html() + return_gameSelect_html() + ' <div class="row-fluid live-game-select">' + return_historyHtml() + return_hotHtml() + ' </div> ' + return_selectAudio_html() + return_selectQuality_html() + ' </div>';
            // <div class="control-group"> <div class="control-label">直播标题：</div> <div class="controls"> <input id="input-liveTitle" type="text"/> </div> </div>
            //<div class="control-group"> <div class="control-label">选择游戏：</div> <div class="controls"> <input id="input-gameName" type="text"/> <ul class="input-search-result none"> </ul></div> </div>

            function return_liveTitle_html() {
                var html = '<input id="input-liveTitle" type="text"/>';
                return return_controlGroup_html('直播标题:', html);
            }

            function return_gameSelect_html() {
                var html = '<input id="input-gameName" type="text"/> <ul class="input-search-result none"> </ul>';
                return return_controlGroup_html('选择游戏:', html);
            }
            ...
            function return_selectAudio_html() {
                var selectDiv = '<div class="mySelect select-audio"> <input type="hidden" id="audio-value"> <div class="selected"></div> <div class="options"></div> </div>';
                return return_controlGroup_html('音频设置:', selectDiv);
            }

            function return_selectQuality_html() {
                var html = '<div class="select-quality"><div class="checkboxDiv"> <label class="checkbox-label checked" for=""> <input type="checkbox" value="high" id="check-quality-high"> </label> <span>高清</span> </div> <div class="checkboxDiv"> <label class="checkbox-label" for=""> <input type="checkbox" value="normal" id="check-quality-normal"> </label> <span>普清</span> </div><div class="clear"></div></div>';
                return return_controlGroup_html('画质选择:', html);
            }

            function return_controlGroup_html(label, body) {
                return '<div class="control-group"> <div class="control-label">' + label + '</div> <div class="controls">' + body + '</div></div>';
            }
        },
        stopNotify: function (data) {
            ...
            var anchorInfo = '<div class="anchor-info"> <div class="face"> <img src="' + user.face + '" alt=""/> </div> <div class="nick-content"> <span class="' + user.level + '"></span> <span class="nick">' + user.nick + '</span> </div> <div class="fansCount">' + user.fans + '</div></div>';

            var live = {};
            live.time = data.liveLong;
            live.viewer = data.userCount;
            live.coin = data.hpcoin;
            live.bean = data.hpbean;
...
function set_user_hpbean(bean) {
    if (pageUser.isLogin) {
        ...
        $('.anchor_money .bean_list span:eq(1)').text(bean)
    }
}
function set_user_hpcoin(coin) {
    if (pageUser.isLogin) {
        coin = digitsFormat(coin);
        $('.anchor_money .coin_list span:eq(1)').text(coin);
    }
}
var is_rm_follow_loading = false;
function rm_follow() {
    if (is_rm_follow_loading) {
        return;
    }
    if (!check_login()) {
        return;
...
function return_videoList_html(d) {
    ...
    var imageClass = (d.orientation == 0 && d.ispic == 1) ? $conf.angleImage : '';
    return '<div class="liveOne"><a href="' + url + '"><div class="imagecontainer"> <img class="' + imageClass + '" src="' + d.poster + '"> <div class="playopt"></div> </div> <div class="liveInfo"> <div class="videoName">' + d.title + '</div> <div class="clear"></div> <div class="liveDetail"> <span class="anchor_icon anchor"></span> <span class="anchor_nick">' + d.nick + '</span> <span class="anchor_icon viewerIcon"></span> <span>' + d.viewCount + '</span> <span class="game_name">' + d.gameName + '</span> </div> </div> </a></div>'
}
...
function silencedUser() {
    ...
    var requestUrl = $conf.api + 'room/setsilentced.php';
    var requestData =  {
        uid: getCookie('_uid'),
        encpass: getCookie('_enc'),
        luid: $ROOM.anchorUserID,
...
!function (a) {
    ...
    function receiveSuccNotice(nick, bean) {
        var content = '<p class="small">恭喜你打开了<span class="nick">' + nick + '</span>的宝箱</p><p class="big">抽取到了<span class="bean">' + bean + '</span>欢朋豆</p>'
            ...
            setTimeout(function () {
                ...
                try{
                    ...
                    if (boxCount <= 0 || waitList.length <= 0) {
                        ...
                        initBox(obj.trid, obj.uid, obj.ctime, obj.unick, $ROOM.treasure.timeOut);
                    }
                }catch(e){
                    console.log(e)
                }
    ...
    function setTime() {
        ...
        if (sCount > 1) {
            ...
            //可领取阶段
            //a('.lr_box_body img').attr('src', 'static/img/gift/treasureBox/box')
            a('.tr_box_time').html('可领取');
            a('#treasure_box_div #treasure-box-content').removeClass().addClass('receive');
        }
    }
...
function updateAnchor_income(gid, num) {
    ...
    if (gid == 31) {
        ...
        $ROOM.anchorIncome += parseFloat(exchangeToBean(num));
        //console.log(exchangeToBean(num));
        //console.log($ROOM.anchorIncome);
...
var airbetter_obj = {
    ...
    betterEffect: function (node_id, index, d) {
        var self = this;
        var html = '<a id="' + node_id + '" href="'+$conf.domain+'room.php?luid=' + d.luid + '" class="air"><div> <div class="air_gif"></div> <div class="title_bg"><span class="nick">' + d.unick + '</span> 送给 <span class="nick">' + d.lunick + '</span> <span class="num">1</span>艘飞船，快来抢宝箱啊!</div><div class="title-after"></div> </div></a>';
...
var chatMessageHtml = {
    ...
    openTreasure:function(d){
        var msg = '打开宝箱，获得了<em style="color:#03a9f4;"> ' + d.num + ' </em>个欢豆<img src="static/img/gift/gift-1.png" style="border-radius:4px; margin:0px 4px;">';
...
!function (a) {
    ...
    chat_obj.liveStart = function (d) {
        ...
        //$('.lr_chat_ul').append("<li class='firstRemend'><span class='noticeName'>[房间公告]</span><span class='lr_userwords noticewords'>直播开始</span></li>");
        chat_obj.chatLiDom().append(chatMessageHtml.notice('直播开始'));
    };

    chat_obj.liveEnd = function () {
        set_report_option(false);
        //$('.lr_chat_ul').append("<li class='firstRemend'><span class='noticeName'>[房间公告]</span><span class='lr_userwords noticewords'>直播结束</span></li>");
    ...
    chat_obj.giftMsg = function (d) {
        ...
        if(d.gid == 31)
            updateAnchor_income(d.gid, d.gnum);
        //欢豆不展示
        if (d.gid > 31)
            gbter_obj.add(d);
    };
    chat_obj.silenceMsg = function (d) {
        chat_obj.errorMsg('用户' + d.targetNick + '被管理员' + d.adminNick + '禁言一个小时');
    };
    ...
    chat_obj.treasureopen = function (d) {
        ...
        //var li = '<li><span class="lr_username alertWarn" data-uid="' + d.uid + '" data-tm="' + d.tm + '">' + d.unick + ': </span><span class="lr_userwords lr_other_userwords"style="padding-left:0px;">打开宝箱，获得了<em style="color:#03a9f4;"> ' + d.num + ' </em>个欢豆<img src="static/img/gift/gift-1.png" style="border-radius:4px;"></span><div class="clear"></div></li>'
        //$('.lr_chat_ul').append(li);
    ...
    chat_obj.netWorkClose = function () {
        ...
        function netWorkLogin() {
            ...
            var uid = getCookie('_uid') || hpRandom(3200000000,3299999999);
            var encpass = getCookie('_enc') || 'gustuserenterencpass';
            var roomid = $ROOM.anchorUserID;
            if (!isNetWorkLogin) {
                runSwfFunction('imProxy', 'login', chatIp, chatPort, uid, encpass, roomid, 'proxyCallBack');
                setTimeout(netWorkLogin, 5000);
            }
        }
    }
}(jQuery);

var msgUpdateTime = new Date().getTime();//纪录心跳信息 用于重连机制
window.proxyCallBack = function (a, b) {
    console.log(a);
    console.log(b);

    if( a == 'Send message called!' && /socket\.connected/.test(b)){
        msgUpdateTime = new Date().getTime();
    }

    if( a=="SecurityError"){
        chat_obj.netWorkClose();
        return;
    }
...
!function (a) {
    ...
    room.init = function () {
        ...
        //swfobject.embedSWF('http://dev.huanpeng.com/main/static/chatProxy.swf', 'imProxy', '1px', '1px', '9.0.0', 'expressInstall.swf');
        //console.log('embed swf in time ' + new Date().getTime());
        //console.log($("#imProxy"));
        var chatIp = $ROOM.chatServer[0].split(':')[0] || '';
        var chatPort = $ROOM.chatServer[0].split(':')[1] || '';
        ...
        room.chat_menushow();
        room.initGiftInfo();
        a('.enlargeBlockDiv').bind('click', function () {
            //console.log('click');
            if (a(this).find('.enlargeBlock').hasClass('addEnlargeBlock')) {
                room.lr_right_show();
            } else {
                room.lr_right_hide();
            }
        ...
        a(window).bind('resize', room.resize);
        ...
        //     var time = new Date().getTime();
        //     if( ( time - msgUpdateTime ) > 50000 && isNetWorkLogin )
        //     {
        //         chat_obj.netWorkClose();
        //     }
        // }, 50000);
    ...
    room.nav_player = function (d) {
        ...
        anchorInfo.find('.game_name').text(d.gameName);
    ...
    room.share_website = function () {
        ...
        dom.shareContent.on('mouseleave', function () {
            setTimeout(function () {
                ...
                //dom.copyBtn.removeEventListener(zClip_afterCopy);
                a('.zclip').remove();
                that.hasCopy = !1;
            }, 100);
        });
    ...
    room.push_phone = function () {
        ...
        $('#qrCode').qrcode({render: 'canvas', text: $conf.domain + 'h5share/live.php?u='+$ROOM.anchorUserID, width: 120, height: 120});
    };
    room.initGiftInfo = function () {
        var gift = $('.liveRoom_opt .giftdiv .gift');
        ...
        var nameList = ['欢豆', '欢朋特饮', '滑板', '黄色小面包', '飞碟'];
        var descList = [
            '快来给主播送欢豆吧',
            '累了，困了，喝欢朋特饮',
            '一步两步，一步两步',
    ...
    room.gift = function () {
        ...
        var shineInterval = {};
        var lwItem;
        $('.lw_item').hover(function () {
            lwItem = this;
            $(this).find('.shine').animate({'left': '67px', 'top': '-7px'}, 500, function () {
                $(lwItem).find('.shine').css({'top': '-78px', 'left': '-3px'});
            });
            shineInterval[$(this).attr('class')] = setInterval(function () {
                $(lwItem).find('.shine').animate({'left': '67px', 'top': '-7px'}, 500, function () {
                    $(lwItem).find('.shine').css({'top': '-78px', 'left': '-3px'});
                });
            }, 5000);
        }, function () {
            $(lwItem).find('.shine').stop();
            $(lwItem).find('.shine').css({'top': '-78px', 'left': '-3px'});
            if (shineInterval[$(this).attr('class')]) {
                clearInterval(shineInterval[$(this).attr('class')]);
            }
        });
    ...
    room.lr_treasure = function () {
        if ($ROOM.treasure.total > 0) {
            var tr_list = $ROOM.treasure.list;
            if(tr_list.length){
                var v = tr_list.shift();
                treasure_obj.initWaitList(tr_list);
                treasure_obj.init(v.trid, v.uid, v.ctime, v.nick, $ROOM.treasure.timeOut);
            }
    ...
    room.lr_other = function () {
        ...
        var tab_con = a('.liveRoomOther .tab_con');
        tab.bind('click', function () {
            tab.removeClass('selected');
            tab_con.addClass('none');
            tab_con.eq($(this).index()).removeClass('none');
    ...
    room.recommend_live_list = function () {
        ...
        ajaxRequest({url:requestUrl,data:requestData},function(d){
            ...
            if (html) a('.liveRoomOther .videoList:eq(0)').html(html);
    ...
    room.anchor_video_list = function () {
        ...
        ajaxRequest({url:requestUrl,data:requestData},function (d) {
            if(d.total > 0){
                ...
                if (html) a('.liveRoomOther .videoList:eq(1)').html(html);
                room.lr_videoList_resize();
            }
        });
    };
    room.anchor_bulletin = function () {
        ...
        ajaxRequest({url:requestUrl,data:requestData},function(d){
            if(d.status == 1){
                $('.liveRoomOther .bulletin').html(d.message);
            }else{
                $('.liveRoomOther .bulletin').html('<div class="nodata" style="width:260px;height:225px;margin: 50px auto;"><img src="'+$conf.domain+'static/img/logo/nodata-bulletin.png'+'" alt=""></div>');
            }
        });
    };
    ...
    room.chat_shieldGift = function(){
        ...
        $(".shieldDiv").bind('click', function(){
            ...
            //     optGroup.removeClass('checked');
			//
            //     $("#giftBetter_item1").show();
            // }else{
            //     shieldGift = true;
            //     dom.addClass('checked');
            //     optGroup.addClass('checked');
			//
            //     if(shieldChatBanner)
        ...
        optGroup.find('.body .checkbox-div').bind('click', function(){
            ...
            //     shieldGift = true;
            //     optGroup.addClass('checked');
            //     $(".shieldDiv .checkbox-label").addClass('checked');
            //
            // }
        ...
        function setValue(name, value){
            try {
                ...
                // eval(param + ' && ' + param +'!=undefined && (' + param + '=' + value + ');' );
            }catch(e){
                console.log('un catch param:');
            }
        }
    }
    room.chat_menushow = function () {
        ...
        $('#black_img').parent().on('click', function () {
            //console.log('jy click');
            silencedUser();
        });
    };
    room.lr_right_show = function () {
        if (a(window).width() <= 1100) {
            $('.liveRoom_left').css('padding-right', '');
        } else {
            $('.liveRoom_left').css('padding-right', '');
        }

        a('#right_block_div').removeClass('none');
        a('.liveRoom_right').css('margin-right', '');
        a('.enlargeBlock').removeClass('addEnlargeBlock');
        a('.enlargeBlock .arrow_left').addClass('arrow_right').removeClass('arrow_left');
        a('.player_gamedesc').css('max-width', $('.liveRoom_nav .player_info').width() - 50 + 'px');
        room.main_resize();
    };
    room.lr_right_hide = function () {
        a('.liveRoom_left').css('padding-right', '15px');
        a('#right_block_div').addClass('none');
        a(".liveRoom_right").css('margin-right', '-350px');
        a('.enlargeBlock').addClass('addEnlargeBlock');
        a('.enlargeBlock .arrow_right').addClass('arrow_left').removeClass('arrow_right');
        a('.player_gamedesc').css('max-width', 'inherit');
        room.main_resize();
    };
    room.lr_chat_resize = function () {
        //var roomHeight = document.body.clientHeight - $('.container-header').height(); //without <!DOCTYPE html>
        var roomHeight = document.documentElement.clientHeight - 48;//48 is container-header height
        roomHeight = roomHeight < 550 ? 550 : roomHeight;
        $('.liveRoom_right').height(roomHeight);

        var banner = $('.liveRoom_right .banner').height() || 0;
        var contri = parseInt($('.liveRoom_right .lr_online').css('margin-top'));
        var chatopt = $('#right_block_div .chatopt').height();
        var write = $('#right_block_div .lr_inwrite').height() + 40;

        var height = roomHeight - banner - contri - chatopt - write;

        $('.liveRoom_right .lr_chat').height(height);

        var lr_opt = a('.liveRoom_opt').width() - 350;
        var task = a('.task').width() + parseInt(a('.task').css('margin-right')) + 20;
        var gtbean = a('.get_hpbean').width() + parseInt(a('.get_hpbean').css('margin-left'));
        var gift = a('.giftdiv').width() + parseInt(a('.giftdiv').css('margin-right'));
        var optcurr = task + gtbean + gift;
        if (optcurr >= lr_opt && $('.enlargeBlock').hasClass('addEnlargeBlock')) {
            $('.liveRoom_right').hide();
        } else {
            $('.liveRoom_right').show();
        }
    ...
    room.lr_topNav_resize = function () {
        var rightspan = $('#topfixed .right .rightspan');

        var max = $('#topfixed').width();
        var left = $('#topfixed .left').width();
        var mid = $('#topfixed .search').width() + parseInt($('#topfixed .search').css('padding-right'));
        var span1 = rightspan.eq(0).width();
        var span2 = rightspan.eq(1).width();
        var span3 = rightspan.eq(2).width();
        var span4 = rightspan.eq(3).width();
        var span5 = pageUser.isLogin ? $('#topfixed .right .userpic').width() + parseInt($('#topfixed .right .userpic').css('margin-right')) + 20 :
        rightspan.eq(4).width() + parseInt(rightspan.eq(4).css('padding-left')) + 10;

        var right = max - left - mid - parseInt($('#topfixed .right').css('margin-right'));
        right = right > 350 ? 350 : right;
        $('#topfixed .right').width(right);

        function resetMargin() {
            rightspan.each(function (index, element) {
                $(element).css('margin-left', '0px');
            });
        }

        if (right - span5 < span4) {
            rightspan.eq(3).hide();
        } else {
            rightspan.eq(3).show();
            resetMargin();
        ...
        if (right - span5 - span4 < span3) {
            rightspan.eq(2).hide();
            rightspan.eq(3).css('margin-left', parseInt(right - span5 - span4) + 'px');
        } else {
            rightspan.eq(2).show();
            resetMargin();
        }

        if (right - span5 - span4 - span3 < span2) {
            rightspan.eq(1).hide();
            rightspan.eq(2).css('margin-left', right - span5 - span4 - span3);
        } else {
            rightspan.eq(1).show();
            resetMargin();
        }

        if (right - span5 - span4 - span3 - span2 < span1) {
            rightspan.eq(0).hide();
            rightspan.eq(1).css('margin-left', right - span5 - span4 - span3 - span2);
        } else {
            rightspan.eq(0).show();
            resetMargin();
        }
    ...
    room.lr_navAnchor_resize = function () {
        var lr_nav = $('.liveRoom_nav').width();
        var lr_nav_player = $('.live_nav_player').width() + parseInt($('.live_nav_player').css('margin-left'));
        var lr_nav_opshow_mright = parseInt($('.live_nav_opshow').css('margin-right')) + 6;

        $('.live_nav_opshow').width(lr_nav - lr_nav_player - lr_nav_opshow_mright);
        ...
        if (sharephone + sharegroup >= shareopt) {
            $('.sharephone').hide();
        } else {
            $('.sharephone').show();
        }

        var att_left = $('.nav_attention_left').width() + 22;
        var att_right = $('.nav_attention_right').width() + 2;
        if ((att_left + att_right) >= $('.live_nav_opshow').width()) {
            $('.nav_attention_left').hide();
        } else {
            $('.nav_attention_left').show();
        }
    };
    room.lr_videoPlayer_resize = function () {
    ...
    room.lr_videoOpt_resize = function () {
        if (!pageUser.isLogin) {
            return;
        }
    ...
    room.lr_videoList_resize = function () {
        ...
        //var lvOne = a('.liveRoomOther .liveOne').width() + 2;
        var count = parseInt(lvOther / lvOne);

        //console.log('video_resize:lvOne'+lvOne);
        //console.log('video_resize:lvOther'+lvOther);
        //console.log('video_resize:count'+count);
    ...
    room.main_resize = function () {
        room.lr_topNav_resize();
        room.lr_navAnchor_resize();
        room.lr_videoPlayer_resize();
        room.lr_videoOpt_resize();
        room.lr_videoList_resize();
        room.lr_chat_resize();
    ...
    room.gust_status = function () {
        ...
        $('.get_hpbean').css('width', 'auto').append('<a class="login-get-hpbean" href="javascript:;" onclick="loginFast.login(0)"><span class="login-img"></span><span class="login-btn">登录领取欢豆</span></a>');
        $('.anchor_money').hide();
    ...
    room.user_status = function () {
        ...
        pageUser.isAnchor && (pageUser.user.userID == $ROOM.anchorUserID) && $_GET['to_open_live'] && pushLiveBox.startLive.init();
    };
    room.anchorToLive = function () {
        ...
        $('.sidebar_list.apply_anchor a').bind('click', function () {
            if ($(this).data('liveStatus') == 'pushLive') {
                ...
                pushLiveBox.startLive.init();
            } else if ($(this).data('liveStatus') == 'closeLive') {
                pushLiveBox.stopLive.init();
            }
        });
    };
    room.initSwfObbject = function () {
        ...
        var wmode = $conf.isIE?'opaque':'window'
        var embedSwfObject = [
            {
                id:'imProxy',
            ...
            },
            {
                ...
                flashVar:{
                    ...
                    "toOpenLive":$_GET['to_open_live'] == 1 ? 1 : 0,
                    'returnDeviceListFuncName':'getDeviceListCallBack',

...
class Video
{
    ...
    function getVideoInfo($videoid)
    {
        ...
        $res = self::$db->field('videoid,uid,gametid,gamename,title,ctime,vfile,orientation,length,poster')->where("videoid=$videoid")->limit(1)->select('video');
        return $res ? $res : '';
    }
    ...
    function getvlistBytype($type, $page, $size)
    {
        ...
        if ($type == 4) {//未通过
            $res = self::$db->field('videoid')->order('ctime  DESC')->limit($page, $size)->select('admin_unpass_video');
        }
    ...
    function getMostVideoInfo($videoid)
    {
        ...
        $res = self::$db->field('videoid,uid,gametid,gamename,title,ctime,vfile,length,poster')->where("videoid in ($videoid)")->select('video');
        if ($res !== false && !empty($res)) {
            foreach ($res as $v) {
                $result[$v['videoid']] = $v;
            }
    ...
    function unPass()
    {
        $res = self::$db->field('count(*) as total')->select('admin_unpass_video');
        if ($res !== false && isset($res[0]['total'])) {

package
{
	...
	public class VideoPlayer extends CommonPlayer
	{
		...
		//输入地址
		protected function inputURL(StreamURL:String, ServerURL:String = null, LiveRoomURL:String = null):void
		{	
			...
			{
				DebugInfoManager.getInstance().log("[CommonPlayer] this.playerCore == null");
			}
			if(this.oldStreamURL == null || this.oldStreamURL == "")
			{
				this.oldStreamURL = StreamURL;
				this.streamURL = this.oldStreamURL;
			}
			else if(StreamURL == this.oldStreamURL)
			{
			}
			else
			{
			...
			if(this.playerCore != null)
			{
				...
				{
					this.playerCore.netConnection.close();
					this.playerCore.netConnection = new NetConnection();
				}
			}
...
//		}
		...
		private function enterFrameFunction(event:Event):void
		{
			...
			//播放完成
			if(this.TimedragSlider != null && this.Timebuttom != null && this.TimedragSlider.x == this.Timebuttom.width)
			{
				this.TimedragSlider.x = 0;
				this.TimeSelected.width = 0;
				this.tfCurrentTime.text = "0:00";
				this.btnPause.visible = false;
				this.btnPlay.visible = true;
				this.playerCore.netStream.seek(0);
				this.playerCore.netStream.pause();
				stage.removeEventListener(Event.ENTER_FRAME, enterFrameFunction);
			}

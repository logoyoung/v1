...
{
    if ($express) $express = time() + $express;
...
//		$errorCode = $result['error_code'];
    ...
    if ($channel != LOGIN_CHANNEL_WEIBO) {
        ...
        if ($userHelp::isUnionidUsed($unionid, $channel, $db)) {
            return -4068;
        } else if ($userHelp::isOpenidUsed($openid, $channel, $db)) {
...
//    dong_log('状态', json_encode(array('status' => $status)), $db);
    if ($row['status'] == LIVE_CLIENT_CREATE && $status > 1) {//对于正在创建中的流 在进行推流结束回调以及录像生成回调 不更改状态

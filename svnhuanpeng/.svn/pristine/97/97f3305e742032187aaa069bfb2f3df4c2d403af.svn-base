...
class DbHelper {
     ...
     */
    public static function getInstance($dbKey,$debug = false) : MysqlConnection
	{
        ...
        try {
            foreach ( self::$inTransaction as $connection) {
                if(!($connection instanceof MysqlConnection)){
                    throw new Exception('must be an instantiated object of a MysqlConnection');
                }
...
class MysqlConnection {
    ...
    protected function logError($e, array $config = []) {
        ...
        if($config){
            $sql     = $this->debugRawSql($config['sql'],$config['bindParam']);
    ...
    private function log($msg,$logName) {
        write_log($msg,APP_NAME.'_'.$logName);
        return ;
    }

...
//		$code = $userObj->checkStateError( $this->_params['encpass'] );
		...
		if ( $info['status'] == self::RECORD_STATUS_SUCCESS )
		{
			...
			$this->_backFormatData( self::RECORD_STATUS_SUCCESS, '', intval( $property['coin']), $info['channelid'], $rmb, $isFirstPay );
		}
...
//		$this->_log( $postData );
  ...
  KEY (`channelid`)
	...
	private function _backFormatData( $status, $errorcode = '', $coin = '', $channelID = '', $cost = '', $isFirstPay = '' )
	{

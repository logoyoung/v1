...
class wsClearDeadLine20170501{
	...
	public function __construct($flvdeadline=null,$mp4deadline=null)
	{
		if($flvdeadline)
			...
			$this->flvdeadline = date('Y-m-d',strtotime('-1 day'));
		if($mp4deadline)
	...
	public function doDeleteFlv()
	{
		...
		foreach ($files as $file){
			$liveinfo = explode('-',$file);
			$liveid = $liveinfo[2];
			if( !empty($liveid) ){
				if(self::checkVideoExists($liveid,$db))
					$deleteFiles[] = $file;
				else
					continue;
			}
	...
	{
		$r = $db->field('liveid,videoid,vfile,status,ctime')->where( 'ctime<\''.$this->mp4deadline.'\'' .' and ctime>\''.self::START_TIME.'\'' . ' and status!='.VIDEO.' and status!='.VIDEO_CLEAR.' and is_save='.self::NO_SAVE )->limit(1)->select('video');
	...
	public static function checkVideoExists($liveid, $db){
		$r = $db->field('vfile')->where("liveid='{$liveid}'")->select('video');
		return isset($r[0]['vfile'])?true:false;
	}

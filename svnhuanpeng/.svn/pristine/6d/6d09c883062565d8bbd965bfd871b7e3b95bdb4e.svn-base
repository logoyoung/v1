...
class [MODEL]Model extends Model {
    ...
    static public function buildRuntime() {
        ...
        mkdir(CACHE_PATH,0777);  // 模板缓存目录
        chmod(CACHE_PATH,0777);  // 模板缓存目录
        if(!is_dir(LOG_PATH)){
            mkdir(LOG_PATH,0777);    // 日志目录
            chmod(LOG_PATH,0777);    // 日志目录
        }
        if(!is_dir(TEMP_PATH)){
            mkdir(TEMP_PATH,0777);   // 数据缓存目录
            chmod(TEMP_PATH,0777);   // 数据缓存目录
        }
        if(!is_dir(DATA_PATH)){
            mkdir(DATA_PATH,0777);   // 数据文件目录
            chmod(DATA_PATH,0777);   // 数据文件目录
        }
        return true;
    }

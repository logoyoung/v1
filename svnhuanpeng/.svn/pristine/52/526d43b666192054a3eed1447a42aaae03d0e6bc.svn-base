...
function getNewLiveLists($redisObj, $db) {
    ...
    if ($getCatch) {
        ...
        $row = $db->field('liveid,uid,poster,orientation,stream')->where("status=" . LIVE)->order('liveid DESC')->select('live');
        if ($row) {
            foreach ($row as $v) {
                $luid[$v['uid']] = $v;
            }
...
function getHotLiveLists($redisObj, $db) {
    ...
    if ($getCatch) {
        ...
        $row = $db->field('liveid,uid,poster,orientation,stream')->where("status=" . LIVE)->select('live');
        if(!$row)
        {
            return $luid;
        }
...
function getLivePicByUid($uid, $conf, $db) {
    $list = array();
    $res = $db->field('liveid,uid,poster,orientation,stream')->where("uid in ($uid)  and status=" . LIVE)->select('live');
    if ($res) {
        getLiveServerList($streamServer, $notifyServer);
        $roomIds=getRoomIdByUid(implode(',', array_column($res, 'uid')), $db);
        foreach ($res as $v) {

...
//    } else {
//       $res = $db->field('uid,gamename,title,poster,ctime,orientation')->where("status= " . LIVE . "  and title like '%$keyword%'")->order('ctime desc')->limit($page, $size)->select('live');
//    }
    if ($res) {
        foreach ($res as $k => $v) {
            $List[$v['uid']] = $v;
        ...
        foreach ($res as $k => $v) {
            ...
            $livelist['posterUrl'] = $v['poster'] ? "http://" . $conf['domain-img'] . '/' . $v['poster'] : VERTICAL;
    ...
    return array('lives' => $livelists, 'total' => $total[0]['total'] ? $total[0]['total'] : '0', 'page' => $page);
}
...
//    } else {
//        $res = $db->field('videoid,uid,gamename,title,poster,vfile,viewcount,orientation')->where("status= " . VIDEO . "  and title like '%$keyword%'")->order('ctime desc')->limit($page, $size)->select('video');
//    }
    if ($res) {
        foreach ($res as $k => $v) {
            $vlist[$v['videoid']] = $v;
    ...
    return array('video' => $vlist, 'total' => isset($total[0]['total']) ? $total[0]['total'] : '0','page'=>$page);
}
...
function search($uid, $encpass, $keyword, $type, $page, $size, $conf, $db) {
    ...
    if ($type == 3) {
        $livelists = getVideobyKeyword($keyword, $type, $page, $size, $conf, $db); //录像
    }
...
$size = isset($_POST['size']) ? ($_POST['size']) : 4;
if (empty($keyword)) {
    error(-987);
}

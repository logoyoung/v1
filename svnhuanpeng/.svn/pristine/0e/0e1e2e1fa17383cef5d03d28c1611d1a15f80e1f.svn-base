...
defined('DOMAIN_PROTOCOL') or define('DOMAIN_PROTOCOL', is_https() ? 'https://' : 'http://' );

//运行环境 目前支持 DEV(开发环境), PRE（测式环境） PRO(生产环境)
define('APP_ENV', isset(get_config(APP_SYSTEM_CONF,'DEV_SERVER')[APP_RUN_HOST_NAME]) ? 'DEV' : (isset(get_config(APP_SYSTEM_CONF,'PRE_SERVER')[APP_RUN_HOST_NAME]) ? 'PRE' : 'PRO'));

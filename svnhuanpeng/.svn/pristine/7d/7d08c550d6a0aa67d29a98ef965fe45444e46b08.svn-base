...
abstract class Driver {
    ...
    protected function initConnect($master=true) {
        ...
        // 数据库重连问题 20160505
        if($limit=C('HP_RECONNECT') and $limit>0){
            for($i=$limit;$i>0;$i--){
                try {
                    if(stripos($this->_linkID->getAttribute(PDO::ATTR_SERVER_INFO),'Uptime:')!==false){
                        return;
                    }
                } catch (\PDOException $e) {
                    //进行重连
                    ...
                    C('HP_RECONNECT',null);
                    $this->initConnect();
                    C('HP_RECONNECT',$limit);
                    \HP\Log\Log::system('SQLReConnect'.$i.':'.$e->getMessage());
                }
            }
            \HP\Log\Log::system('SQLReConnect_FAIL:'.$e->getMessage());
        }
    }

...
class UserEvent extends EventAbstract
{
    ...
    private $_errorLog = 'user_event_error';
    private $_infoLog  = 'user_event_info';
    ...
    public function trigger($action,$param)
    {
        ...
        {
            write_log("error|redis服务异常，不作事件处理action:{$action}; param:".hp_json_encode($param),$this->_errorLog);
            return false;
        }
    ...
    private function _checkUid()
    {
        ...
        if(!$phone)
        {
            throw new Exception('invalid uid', 500);
        }
    ...
    private function _initUserActiveParam()
    {
        ...
        if($data === false)
        {
            write_log("error|从数据库获取useractive 异常;uid:{$uid};",$this->_errorLog);
            return false;
        }
    ...
    private function _initUserDisableLoginParam()
    {
        ...
        if($result === false)
        {
            write_log("error|从数据库获取user_disable_status 异常;uid:{$uid},type:{$type},scope:{$scope};",$this->_errorLog);
            return false;
        }
    ...
    private function _updateBindPhoneToUidRedis()
    {
        ...
        write_log($log,($result === false) ? $this->_errorLog : $this->_infoLog);
    ...
    private function _updateSilencedRedisData()
    {
        ...
        if($result === false)
        {
            write_log("error|uid:{$uid};重新构造用户直播间禁言,数据库异常",$this->_errorLog);
            return false;
        }
        ...
        if(!$result)
        {
            ...
            write_log($msg,($r ? $this->_infoLog : $this->_errorLog));
            return true;
        }
        ...
        {
            write_log("success|uid:{$uid};重新构造用户直播间禁言成功",$this->_infoLog);
            return true;
        }

        write_log("error|uid:{$uid};重新构造用户直播间禁言,redis异常",$this->_errorLog);
        return false;
    }
    ...
    private function _updateAnchorDisableLiveRedis()
    {
        ...
        {
            write_log("error|从数据库获取主播禁播数据异常", $this->_errorLog);
            return false;
        }
        ...
        if(!$disableLiveDbData)
        {
            ...
            {
                write_log("success|构建禁播数据成功;uid:{$uid}", $this->_infoLog);
                return true;
            }

            write_log("error|构建禁播数据redis异常;uid:{$uid}", $this->_errorLog);
            return false;
        }
        ...
        write_log("error|构建禁播redis异常;uid:{$uid}", $this->_errorLog);
        return false;
    }

...
class PublicController extends BaseController
{
    ...
    {
        if(IS_AJAX){
             $username = trim(I('post.u'));
             $password = trim(I('post.p'));
             if(empty($username) || empty($password)){
                 return $this->ajaxReturn(['status'=>1,'msg'=>'参数不全!']);
             }
             if(!C('INTRA_IP') and !(new \Think\Verify())->check(trim(I('post.v')))){
...
//            'useNoise'=>false,
	...
	public function gamelist()
	{
		...
		$results = $dao->select();
		foreach ($results as $result){
			$data['value'] = $result['name'].'|'.$result['gameid'];
			$data['id'] = $result['gameid'];
			$datas[] = $data;
		}
		return $this->ajaxReturn( $datas );
	}
    ...
    {
    	$results = D('promocode')->select();
    	foreach ($results as $result){
    		$data['value'] = $result['name'].'|'.$result['promocode'];
    		$data['id'] = $result['promocode'];
    		$datas[] = $data;
    	}
    	return $this->ajaxReturn( $datas );
    }

...
class Page{
    ...
    protected $config  = array(
        'header' => '<li class="rows"><a href="javascript:;" onclick="Op_common.alert(\'共%TOTAL_ROW%条记录,分为%TOTAL_PAGE%页!\');">总共: %TOTAL_ROW%</a></li>',
        'prev'   => '上一页',
        'next'   => '下一页',
        'first'  => '1...',
        'last'   => '...%TOTAL_PAGE%',
    ...
    public function show() {
        ...
        preg_match('/[\?\&]'.$this->p.'=\d*/',$this->url,$match);
        if($match){
            $this->url = strtr($this->url,array($match[0]=>substr($match[0],0,1).$this->p.'=[PAGE]'));
        }else{
            $this->url .= (strpos($this->url,'?')===false?'?'.$this->p.'=[PAGE]':'&'.$this->p.'=[PAGE]');
        }
        /* 计算分页信息 */

...
class CronMonitor
{
    ...
    //日志目录
    const CRON_RUN_LOG_DIR = '/data/logs/crontab/hpframe_task/';
    //cron 配置文件
    const CRON_CONFIG_FILE = self::APP_CONFIG_DIR.'crontab.%s.ini';
    ...
    private function initCronConfData()
    {
        ...
        if(!$conf)
        {
            ...
            $conf = parse_ini_file_multi(self::getConfigFile(),true);
            if(!$conf)
            {
                return false;
            }
        ...
        foreach ($conf as $name => $v)
        {
            ...
            if(!isset($v['crontab']) || !$v['crontab'])
            {
                $this->log("error|请配置crontab :name:{$name}:param:".json_encode($v));
                continue;
            }
            ...
            if(!isset($this->cronConfData[$name]))
            {
                ...
                if(!$localHostName || !isset($v['server']) || !in_array($localHostName, $v['server']))
                {
    ...
    {
        return sprintf(self::CRON_CONFIG_FILE,strtolower(app_env()));
    }

...
class RedisConnection {
    ...
    protected $qt           = 0.2;
    protected $accessLogFile = 'redis_access';
    protected $errorLogFile  = 'redis_error';
    ...
    public function __construct(array $config,$name,$debug = false) {
        ...
        $this->debug        = $debug ? true : (isset($config['debug']) && $config['debug'] ? true : false);
        $config             = null;
        $name               = null;
    }
    ...
    public function connect($config) {
        ...
        {
            static $_s  = [];
            if (isset($_s[$connectionKey]) && $_s[$connectionKey] === false)
            {
                return false;
            }
        }
        ...
        if(isset($this->connection[$connectionKey])){
            ...
            try {
                ...
                $this->accessLog('notice|connect timeout'.$e->getMessage(),$config);
            }
        ...
        if(!$status){
            $this->errorLog("error|connect error connection_time:{$t}s",$config);
            $status = null;
            if (!IS_CLI)
            {
                $_s[$connectionKey] = false;
            }
        ...
        if($this->debug || $t >= $this->qt){
            $this->accessLog("connection_time:{$t}s",$config);
        }
    ...
    public function __call($method, $args){
        ...
        do {
            ...
            try {
                ...
                if($this->debug || $t >= $this->qt){
                    $this->accessLog("excute time:{$t} method:{$method} master:{$config['master']}",$config,$args);
                }
                ...
                if(isset($config))
                {
                    ...
                    if($retry <= 0 || Cmd::isWriteCmd($method)){
                        $this->errorLog("error|excute error msg:{$e->getMessage()},master:{$config['master']}",$config,$args);
                        break;
                    }

                    $this->errorLog("warning|retry msg:{$e->getMessage()},master:{$config['master']}",$config,$args);
                } else
                {
                    $this->errorLog("warning|retry msg:{$e->getMessage()}",[],$args);
                }
    ...
    public function getConfigByKeyCmd($key,$cmd,$master = 0) {
        ...
        if($clusterCount < 1 ) {
            $this->errorLog("empty redis config key:{$key},cmd:{$cmd},master:{$master}");
            throw new Exception("empty redis config key:{$key},cmd:{$cmd},master:{$master}");
        }

        $serverNum   = ($keyHashInt % $clusterCount);
        $config      = isset($cluster[$serverNum]) ? $cluster[$serverNum] : (isset($cluster[0]) ? $cluster[0] : [] );
        if(!$config) {
            $this->errorLog("empty redis config key:{$key},cmd:{$cmd},master:{$master}");
            throw new Exception("empty redis config key:{$key},cmd:{$cmd},master:{$master}");
        }
    ...
    private function accessLog($msg = '',$config = [],$param = []) {
        write_log($this->buildLogParam($msg, $config, $param),$this->accessLogFile);
    }

    private function errorLog($msg = '',$config = [],$param = []) {
        $msg = $this->buildLogParam($msg, $config, $param);
        write_log($msg,$this->errorLogFile);
    }

    private function buildLogParam($msg = '', $config = [], $param = []) {

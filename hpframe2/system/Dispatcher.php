...
class Dispatcher
{
    ...
    public function run()
    {
        ...
        {
            throw new Exception(" Class ".APP_NAMESPACE."{$this->match['controller']} not found",self::ERROR_CONTROLLER_INVALID);
        }

        $controller = new $this->controller();
        if(!is_callable([$controller,$this->action]))
        {
            throw new Exception(" Call to undefined method {$this->action} in class: ".APP_NAMESPACE."{$this->match['controller']} ",self::ERROR_ACTION_INVALID);
        }
        ...
        if($this->callback)
        {
            ...
            {
                 if(!call_user_func_array($call, [$this]))
                 {
                    throw new Exception('boot event error',self::ERROR_BOOT_EVENT);
                 }
            }
        }
    ...
    {
        return $this->match['controller'].'/'.$this->match['action'];
    }
}